...
%  \] 
 ...
 The adaptivity definition and the estimated adaptivity are both inaccurate.
    ...
    \\
    The first reason comes from the trace in the operational semantics, where some important information is lost.
    %   in-precision in formalizing the \emph{adaptivity}.
    %  generation.
    %   generated through the operational semantic rules.
    The trace only tracks the query requests, so it loses the execution history of the variables
