...
% we need to be able to identify whether two queries are the equivalent or not, so that when we change the result of one query, whether or not another query is affected. 
...
% We also have the operator $\tlabel : \mathcal{T} \to \ldom$, which gives the set of labels in every event belonging to a trace. 
...
% \jl{We introduce some operations here: the trace concatenation $\tracecat: \mathcal{T} \to \mathcal{T} \to \mathcal{T}$, which combines two traces; the belong to operator $\in$ so that an event $\event \in \eventset$ belongs to a trace $\trace$ is notated by $\event \in \trace$. 
% As usual, we denote by $\event \notin \trace$ that the event $\event$ doesn't belong to the trace $\trace$. 
% Another operator $\llabel : \mathcal{T} \to \mathcal{VAR} \to \{\mathbb{N}\}\cup \{\bot\}$,
% takes a trace and a variable and returns the label of the latest assignment event which assigns value to that variable. We also have the operator $\tlabel : \mathcal{T} \to \mathcal{P}{(\mathbb{N})}$, which gives the set of labels in every event belonging to a trace. The full definitions of these above operators can be found in the appendix.
% }
% \wq{It seems trace concatenation and event belongs to a trace does not deserve so many space here:-)}
%\jl{I agree}

% \\
% We also introduce a counting operator $\vcounter : \mathcal{T} \to \mathbb{N} \to \mathbb{N}$, 
...
% \wq{Question: Seem $\env$ is a function which looks up in the input trace and returns you the latest value of the variable. I have a question, in the two round example, i see $env(\tau)(k)$ while $k$ is not defined(it is input), so in our two round example in Overview, the value stores in the second event is $\bot$?   Also, another important, $\env$ relies on the input trace, so it will not appear in the trace, or config, is it precise?}
% \jl{yes, it is precise. 
% We have initial trace and everything belong is defined over all possible initial trace.
%in the two-round example, there is an intial trace where the value of k is defined there. It is worth to explain this here.
% }
\[
\begin{array}{lll}
...
We give a selection of rules of the trace-based operational semantics in Figure~\ref{fig:os}. 
...
\inferrule{ 
 ...
 \end{mathpar}
% }
% \wq{The rules for if hand while both have two versions, when the guard evaluates to true and false, respectively. In these rules, the evaluation of the guard also generates testing event and our trace is updated as well. }
...
% % it is possible that the event actually showing up in this trace has different form of query value, but they are equivalent by Definition~\ref{def:query_equal}.
% Since the equivalence over two events is defined over the query value equivalence, 
