...
% for example whether the secrete data is leaked in security area.
...
on the program's quantitative property. 
...
%  the performance in terms of execution time of two pieces of code for the same task can be used to decide which one to use. 
...
%  In Resource Cost Analysis Area, the Reachability-Bound is an execution property has broad
% applications in bounding resources cost by a program or analyzing program's other run-time behaviours.
