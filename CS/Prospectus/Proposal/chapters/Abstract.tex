...
such as
the dependency of the program's outputs and inputs
(expressed in terms of reachability of the input data), 
the secret data's leakage in the program (reachability of the secret data), 
whether some pieces of code are executed (reachability of the code pieces), etc.
}
...
%  for different potential applications 
 than only one.
For example, knowing both the reachability and reaching times of the pieces of code
can better help to optimize the software in resource usage;
knowing both the reachability and reaching times of the secret data can better help
to improve the software's security; etc.
}
% Moreover, 
...
However, it can not tell us how much data is leaked.
% area.
...
the new program analysis framework analyzing several quantitative reachability properties of programs
...
The first quantitative reachability property
...
this intuitive \emph{adaptivity} are not always clear in the program
when they want to use them to control the programs' generalization errors.
\highlight{In order to assist them to control the generalization error automatically and efficiently,
...
%   based on dependency graphs,
 provides the data analysts with an upper bound on their program's \emph{adaptivity}.
...
%  a program analysis framework for 
 estimates the program's \emph{adaptivity} statically.
 }
