...
because they are not in the sample data but still provided with the personalized services by using the generalized analysis results.
...
\highlight{
    High-quality
service in people's mobile device life does not just rely on the accuracy of service but also
the efficiency of the service. 
This brings my attention to another non-functional property, the program's resource cost.
...
We observe that the non-functional properties of resource usage are usually
related to two aspects, reachability and quantity.
Providing information on these reachability quantitative properties
can help to control the reachability of some resources and improve the program reliability,
or specify certain bounds on the target resource to guarantee performance. 
