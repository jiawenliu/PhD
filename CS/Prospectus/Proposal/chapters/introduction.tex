...
% I introduce the background and limitations of the 
...
% Targeting the three challenges, I introduce the methodologies 
...
% %
% Based on the implementation and experimental results of this analysis framework, 
...
Program analysis analyzes the properties of a computer program.
% \wq{computer programs with respect to some properties, such as correctness, by analyzing the execution correctness behavior } .
...
% while not enough effort was put on, a variety of those non-function properties, 
% considering its wide potential applications in modern society.
% % \wq{i think you can remove the next sentence.}
% A few decades ago,
% % computer programs were only used for research or business purpose and 
% the expectation of computer programs was just to execute correctly without bugs. 
% \highlight{
 Much attention in the programming language community has focused on the functional correctness property.
...
% A variety of non-functional program properties are more important in modern society than functional correctness property.
% considering its wide potential applications in modern society.
% while not enough effort was put on, 
% \wq{i think you can remove the next sentence.}
% A few decades ago,
% % computer programs were only used for research or business purpose and 
% the expectation of computer programs was just to execute correctly without bugs. 
...
% service in daily life through these programs.
% through mobile devices.
% when running on a mobile device and handling big data.
% \\
 In this sense, the non-functional properties
 come into play in the new era.
% \wq{I think you can remove this paragraph.}
%  }
...
% service
% \wq{which service?}
% aims to provide users with an accurate personalized recommendation.
is determined largely by
the recommendation accuracy when using applications.
% of their personalized recommendation results.
% for every user.
% \wq{Previous sentence is not easy to follow.}
...
% \wqside{the first behaviour of programs, it sounds weird by using the first? then, what is the second?}
% i.e., the accuracy quality.
% of the
% \wq{do you specify certain results, if so, use the} data analysis results.
% in their machine learning algorithms.
% comes from the machine learning area which is popular and widely applied in our daily mobile life.
% In this area,
...
% first reachability quantitative properties 
...
% \highlight{
% A high-quality
% service in people's mobile device life
High-quality modern daily life
% \wq{mobile device life sounds ...} 
is not only
...
% \wq{"Code" in the sense of "computer code" is a mass noun, and so has no plural. 
% It would be "lines of code". 
% In other senses (such as "code" meaning an encrypted system of communication or an encrypted message), 
% it is a countable noun and can be pluralized as normal. }
are executed.
...
% \wq{previous sentence is not clear. }
...
% and the data\wq{the data? which data?}.
...
% \wq{service is plural?} 
% on mobile devices.
% To this end, the execution times of certain pieces of code are
% the second reachability quantitative property I plan to analyze in this proposal.
...
analyses
% frameworks\wq{you have two frameworks?}
% into the new
% analysis frameworks 
...
% \wq{estimate a property? maybe estimate is not the right word here}
% }
...
% \\
% The work in paper "Non-cumulative Resource Analysis" gave a generic resource analysis framework for today’s imperative language enriched with instructions to acquire and release resources. 
...
% Moving towards the area of the program's quantitative property analysis,
...
\begin{enumerate}
 \item \redd{PART I}: A program analysis framework for analyzing the adaptivity of the program that implements an adaptive data analysis.
