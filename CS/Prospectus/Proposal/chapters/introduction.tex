...
One of the most significant properties with respect to programs is the execution correctness,
which determines whether a program executes correctly without getting stuck because of bugs.
 The execution correctness can be proved
%  \wq{prove a behaviour?} 
 by showing the functional correctness property of the
%   \wq{the?} p
program with the help of some formal verification methods such as type systems and program logic.
...
\highlight{
    Much attention in the programming language community has focused on the functional correctness property.
    However, the expectation on the computer programs has changed along with the popularity of mobile devices and wide applications of big data. 
A variety of non-function properties are more important in modern society than the functional correctness property.
% considering its wide potential applications in modern society.
...
For example, to provide people with high-quality
...
service
% \wq{which service?}
% aims to provide the users with accurate personalized recommendation.
is determined largely by the accuracy of the personalized recommendation results for every user.
...
The accuracy of the recommendation results relies heavily on the
% \wq{the?} 
data analysis results over big data.
% These analysis results are used to provide the users with accurately personalized services.
This brings my attention to
% on\wq{(to?)} 
one of the non-functional properties of the
% \wq{remove the?} 
...
%  of the
% \wq{do you specify certain result, if so, use the} data analysis results.
...
analyzes some sample data to get some generalized results
...
is not
determined only by the accuracy of recommendation results,
but also
the performance, 
% the\wq{remove the?} 
privacy, etc., of the service. 
...
% it is a countable noun and can be pluralised as normal. }
are executed.
...
and the execution times of them.
% \\
This brings my attention to another non-functional property,
the reachability and execution times of certain lines of program code.
...
% \wq{estimate a property? maybe estimate is not a right word here}
}
