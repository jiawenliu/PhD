...
\\
There are two cases, 
\subsubcaseL{$ (\forall z^r \in \ldom \sthat     
...
% \exists z^r \in \mathbb{LV}_{\diff} \sthat   \flowsto(z^r, y^j, c)$.
\\
...
% $\forall z^r \in FV(\pi_1(\event_b)) \sthat   \flowsto(z^r, y^j, c)$.
\\
...
% we know $
% \exists z^r \in ((\mathbb{LV}_{\diff} \cup \{x^i\} ) \cap FV(\pi_1(\event_b))) \sthat  \flowsto(z^r, y^j, c_2)$, i.e., 
% \\
% $\flowsto(x^i, y^j, c) \lor 
% \exists z^r \in \mathbb{LV}_{\diff} \sthat   \flowsto(z^r, y^j, c)$.
\\
