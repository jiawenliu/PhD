...
\\
To show 
$(c1)~\exists \event_b = (b, i, v, \bullet) \in \trace_1 \sthat   \neg \event_b \in \trace_2
\land 
(c2)~\forall z \in FV(b), \exists l \in \ldom \sthat   \flowsto(z^l, y^j, c)$
\\
Splitting the conjunction, there are two sub conclusions need to be proved:
\\
\textbf{Subproof of $\boldsymbol{(c1)~\exists \event_b = (b, i, v, \bullet) \in \trace_1 \sthat   \neg \event_b \in \trace_2}$}:
\\
To show $(c1)$
% $(c1)~\exists \event_b = (b, i, v, \bullet) \in \trace_1 \sthat   \neg \event_b \in \trace_2$, 
it is sufficient to show a contradiction by assuming 
\\
$(hc)~\neg (\exists \event_b = (b, i, v, \bullet) \in \trace_1 \sthat   \neg \event_b \in \trace_2)$.
\\
By $(hc)$, we know there are two cases
\\
(1) $\forall \event \in \trace_1 \sthat   \event \in \eventset^{\asn}$.
\\
(2) $\forall \event_b = (b, i, v, \bullet) \in \trace_1 \sthat   
\event_b \notin \trace_2$.
%
\subcaseL{(1) $\forall \event \in \trace_1 \sthat   \event \in \eventset^{\asn}$}
...
% \\%
\subcaseL{(2) $\forall \event_b = (b, i, v, \bullet) \in \trace_1 \sthat   
\neg \event_b \notin \trace_2$}
% \todo{rewrite}
...
% To show $(c2)~\forall z \in FV(b), \exists l \in \ldom \sthat   \flowsto(z^l, y^j, c)$,
% we know from $(c1)$ such that there exists $\event_b = (b, i, v, \bullet) \in \trace_1 \sthat   \neg \event_b \in \trace_2$.
\\
By $(\diamond)$ and $(\star)$,
