...
\begin{proof}
	...
	\caseL{1}
		...
		is executed but the value assigned to the variable in this event is changed in second execution).}
	\input{chapters/appendices/thm-adaptivity/base_case_val}
	% \end{case}
	\caseL{2}
		...
		%
		 ...
		 3. By $\flowsto$ definition we have:
		   $\flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c)$
		\input{chapters/appendices/thm-adaptivity/base_case_ctl}
\end{proof}
%
...
%
	\[
	\begin{array}{l}
		\forall D \in \dbdom , c \in \cdom, \trace \in \mathcal{T} \sthat \forall \event_1, \event_2 \in \eventset^{\asn} \sthat
...
\[
\begin{array}{l}
	\forall D \in \dbdom , c \in \cdom, \trace \in \mathcal{T} \sthat \forall \event_1,\in \eventset^{\asn}, \event_2 \in \eventset^{\test} \sthat
	 \exists \trace' \in \mathcal{T} \sthat \trace = [\event_1] \tracecat \trace' \tracecat [\event_2]
	\implies
	\eventdep(\event_1, \event_2, \trace, c, D) 
	\\ \quad 
...
taking arbitrary trace has the form $\trace = [\event_1; \cdots; \event_2]$ for arbitrary $\trace_2 \in \mathcal{T}$,
 then we know $\exists \trace' \in \mathcal{T} \sthat \trace = [\event_1] \tracecat \trace' \tracecat [\event_2]$, let $\trace_2$ be this $\trace'$.
%
...
%   \trace_2' \in \mathcal{T}, c_1, c_2 \in \cdom \sthat
% \]
%
\[
\begin{array}{l}
...
%
\[
	\forall x^i \in \lvar, \trace, \trace' \in \mathcal{T}, v, v' \sthat
	...
	\]
	\[
		\forall x^i \in \lvar, \trace, \trace' \in \mathcal{T}, \qval, \qval' \sthat
		\Big( \forall z^j \in \lvar / \{\pi_1(\event_z)^{\pi_2(\event_z)} \} \sthat 
		\env(\trace) z = \env(\trace') z \Big) \land 
...
$
	...
	\trace, \trace' \in \mathcal{T}, v, v' \sthat 
...
$
	\forall z^l \in \lvar \setminus ( \lvar_{\trace_2} \setminus \lvar_{\kw{use}_{\trace_2}}), 
	\trace, \trace' \in \mathcal{T}, \qval, \qval' \sthat 
	\env(\trace) z = \env(\trace') z 
	\land 
...
% \[
% 	\forall z^j \in (\lvar \setminus \mathbb{LV}_{\diff_{\eventset}} ), \trace, \trace' \in \mathcal{T}, v, v' \sthat 
% 	\env(\trace) z = \env(\trace') z 
% 	\land 
% 	\config{\trace, \expr_2} \aarrow v 
% 	\land 
...
\end{proof}
%
\input{chapters/appendices/thm-adaptivity/lem_basicinversion}
%
...
% \[
% \begin{array}{l}
%     \forall D \in \dbdom , c \in \cdom, \trace \in \mathcal{T}, l \sthat \forall \event_1, \event_2 \in \eventset^{\asn} \sthat
%   \\ \quad 
...
% 		% \left(\left\{[\event_2]\right\} \cup \left\{ \event_1 \sthatackrel{[\event_1; \event_2]}{\uplus} [\event_2] \right\} \right))
...
% \[
% 	\begin{array}{l}
% 	% \event_1 \sthatackrel{[\event_1; \event_2]}{\uplus} [\event_2]
...
% 		\exists \trace, \trace' \in \mathcal{T} \sthat 
% 		\\ \quad
...
then $\eventdep(\event_1, \event_3, \trace_{12}\tracecat\trace_{23}, c, D)$.
  ...
  \[
	  \begin{array}{l}
  \forall D \in \dbdom , c \in \cdom, \event_1, \event_2, \event_3 \in \eventset^{\asn}, \trace_{12}, \trace_{23} \in \mathcal{T} \sthat 
