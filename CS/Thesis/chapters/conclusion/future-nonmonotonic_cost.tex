...
they design the effect systems~\cite{CicekBG0H17,RadicekBG0Z18,QuG019},
Hoare-Logic~\cite{GaboardiKOS21}, or amortized type system~\cite{RajaniG0021}.
...
The amortized type-system designed in \cite{RajaniG0021} considers a perfect garbage collection
...
The new analysis framework is based on adopting the program analysis framework for adaptivity in PART I and combining the new path-sensitive reachability bound algorithm in PART II
...
\begin{enumerate}
 ...
 The query annotation is replaced by operator annotation $\traceOP$ for whether the variable is assigned by $\alloc$ or $\release$ as follows.
 \highlight{
 \[
 \traceOP \triangleq
...
\begin{enumerate}
 \item The analysis processes in the first two steps of $\THESYSTEM$ in Section~\ref{sec:adapt-static}
 and Section~\ref{sec:alg_weightgen} will be adopted exactly the same in the generalized $\THESYSTEM$.
 \item Then, generalized $\THESYSTEM$ will construct a similar estimated dependency graph 
 in the same way as in Section~\ref{sec:adapt-static}. 
 In the same way as defining $\traceOP$, 
 the estimated graph is enriched with the new annotation for describing the non-monotonic property.
 \item Then, based on the reachability bound analysis result, the weight in the estimated graph
 now describes this resource cost quantity,
  through the new non-monotonic length definition (similar to Definition~\ref{def:NMlen}) on the finite walk.
