...
Our semantics-based method formalizes this intuitive \emph{adaptivity} as a \emph{quantitative reachability} program property semantically as follows.
\begin{enumerate}
   \item We firstly semantically define the \emph{dependency relation}, $\vardep(c, x^i, y^j)$ for variables $x^i, y^j$ in the program $c$, inspired from the method in~\cite{Cousot19a, ZanioliC11, CheneyAA11}.
   This relation can be used to reason about dependence relation between queries.

   \item Then we define the \emph{dependency quantity} 
     based on the \emph{dependency relation} above, which is inspired from resource cost analysis techniques and resource bound analysis works.

   \item Then we construct a dependency graph by combining both the \emph{dependency relation} and \emph{dependency quantity} property on it. This dependency graph accurately reflects the information of intuitive \emph{adaptivity}.
...
\item Then
{\THESYSTEM} combines the \emph{dependency quantity} and \emph{dependency relation}
