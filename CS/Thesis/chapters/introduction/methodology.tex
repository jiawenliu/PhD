...
There is also purely syntactic-based methodology originated from~\cite{Weiser84} and developed in \cite{DenningD77} in defining a property (the data dependency relation), but it suffers the limitation of certain language and is not easy to generalize into other setting.
% dependency analysis. 
...
Because of its nature as a \emph{quantitative reachability} property, it requires us to defining both the dependency property and the quantitative property for the program.
...
applications in analyzing other program property beyond \emph{quantitative reachability}.
...
The first key idea -- the \emph{path reachability-bound} $\inoutB(\rprog, \tpath)$ is a reachability-bound for a loop free path $\tpath$ inside a refined loop $\rprog$. This bound approximates the evaluation times of each loop free path instead of the entire multipath loop.
\\
...
The \emph{loop reachability-bound}, $\lpchB(L:\rprog, \tpath)$ is our second key novelty.
