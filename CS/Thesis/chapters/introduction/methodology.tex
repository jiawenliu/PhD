...
% \todo{Brief Describe our Language Model}
...
The first novel quantity -- the \emph{path reachability-bound} $\inoutB(\rprog, \tpath)$ is a reachability-bound for a loop free path $\tpath$ inside a refined loop $\rprog$. This bound approximates the evaluation times of each loop free path instead of the entire multipath loop.
\\
...
in one iteration of the outside loop.} \cite{GulwaniJK09} considers a similar amount based on \emph{Progress Invariant}, but it is only able to compute
the
