...
\begin{enumerate}
   ...
   This relation capture both the control influence and value influence between data, which are never fully captured in previous dependency relation.
   This relation can be used to reason about the dependency between queries.

   \item Then we define the \emph{dependency quantity} with respect to each variable $x^i$
     based on the \emph{dependency relation}, which is inspired from the works in the program complexity and resource cost analysis areas. This quantity captures a sub-property of \emph{adaptivity} from the quantitative perspective.
...
\paragraph{New Methodology}
We develop a static program adaptivity analysis framework, named {\THESYSTEM} in Chapter~\ref{sec:adapt-static}.
...
and constructs an \emph{estimated dependence} graph. This combination is never consider before, and it complements the weakness of both lines of works. In comparison to purely use single analysis, 
...
The \emph{path reachability-bound} and the \emph{loop reachability-bound} are the main novelty of our new path-sensitive reachability-bound algorithm build on the traditional methods.
