...
% To this end, this thesis focus on analyzing the program's \emph{quantitative-reachability}. 
...
service in people's mobile device life than just from single view. 
% By exploiting the \emph{quantitative-reachability},
...
% In this thesis, we analyze two \emph{quantitative-reachability} properties of programs that can be used to improve programs' performance.

As the first property, we want to improve the data analysis results and reduce its generalization error.
%  High generalization error makes the analysis result of these programs not reliable.
...
we look into 
% especially when the data analysis is adaptive.
it as the first \emph{quantitative-reachability}.

\highlight{
As the second \emph{quantitative-reachability} property,
we study the program's \emph{reachability-bound}.
...
combining the reachability and quantitative analysis techniques to effectively find a precise symbolic reachability-bound on the program's every control location.
}
