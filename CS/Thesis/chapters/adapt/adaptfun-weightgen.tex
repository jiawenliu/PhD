...
Then, for each $(x^l, w) \in \progW(c),\vtrace_0 \in \ftdom_0(c), \trace \in \ftdom,
...
\config{{c}, \trace_0} \to^{*} \config{\clabel{\eskip}^{l'}, \trace_0 \tracecat\vtrace} 
\land 
\config{\vtrace_0, \hat{w}(x^l)} \earrow v
\land
\implies
...
\config{{c}, \trace_0} \to^{*} \config{\clabel{\eskip}^{l'}, \trace_0 \tracecat\vtrace} 
\land 
\config{\vtrace_0, \hat{w}(x^l, c)} \earrow v
\land
\implies
w(x^l, c)(\trace_0) \leq v
\end{array}
\]
\end{thm}
