...
\end{array}
\]
$\kw{SCC}(\absG(c))$ is the set of all the strongly connected components of $\absG(c)$.
\end{defn}
...
To compute these two quantities we use two mutually recursive procedures in a path-insensitive manner as follows.
...
Definition~\ref{def:ranking_bound} estimates the upper bound, $\varinvar(x, c) \in \mathcal{A}_{\lin}$
...
This computation involves computing the \emph{transition bound} $\absclr(\absevent, c)$ of an edge $\absevent$ as below.
...
command and the edge $\absevent = (l, \_, \_) \in \absG(c)$ has the same label,
% label $l \in \lvar(c)$,
then its $\absclr(\absevent, c)$ 
 is a sound upper bound on 
% execution-based reachability bound $w^t$ 
the execution times of this assignment command with label $l$ when $c$ is executing under any
...
\end{defn}
%
Notice that $\hat{w} \in \scexprdom$ is a symbolic arithmetic expression over symbols in $\constdom$. In particular, it may contain the input variables and so it may effectively be used as a function of the input - and capture loop bounds in terms of these inputs.
 
We guarantee that this 
symbolic expression for $x^l \in \progV(c)$ is a sound upper bound of 
the weight for the same vertex $x^l$ in Program's semantics-based dependency graph as follows.
...
In the $\kw{ifQuery}$ program in Figure~\ref{fig:abscfg_ifQuery}(a), there is not a loop.
Every vertex is visited at most once, and we estimate its weight as in the
superscript of each vertex in Figure~\ref{fig:abscfg_ifQuery}(c).

\paragraph{Example III.}
In the $\kw{loopQuery}$ program in Figure~\ref{fig:abscfg_loopQuery}(a), 
the while loop will iterate the number of $Q_m$ times, where $Q_m$ is a symbol representing the maximal value of a query answer.
In this sense, we estimate the weight for vertices in the loop body as $Q_m$ in the
superscript of each vertex in Figure~\ref{fig:abscfg_ifQuery}(c).
This $Q_m$ is indeed similar to the $\infty$ since we do not have a finite bound on the possible answer returned from the query requests.
We construct this weight in order to have a sound approximation on the semantics-based dependency graph.