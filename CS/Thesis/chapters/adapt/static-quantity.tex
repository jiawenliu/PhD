...
% \]
% \end{thm}
\paragraph{The Dependency Quantity Estimation for Labeled Variables}
% The weight for each vertex in $\progV(c)$ is computed as follows,
The dependency quantity for every labeled variable $x^l \in \lvar(c)$ is estimated as the reachablility bound for the location of the same label
from the $\absW(c)$.
\\
Again, as introduced in third step of this analysis framework in Section~\ref{sec:static-overview}, we also build the
weight and vertex set here. In order to estimate the execution-based dependency graph for the program.
\\
...
The same way for the rest weights' computation.
%
\paragraph{The Dependency Quantity Estimation for the Pair of Labeled Variables}
The dependency quantity for every pair of labeled variables $x^i, y^j \in \lvar(c)$ is also estimated
as the minimum reachability bounds for the locations having the same label $i, j$
from the $\absW(c)$.
\\
Again, as introduced in third step of this analysis framework in Section~\ref{sec:static-overview}, we also build the
weight for every edge here. In order to estimate the execution-based dependency graph for the program.
% The weight for each vertex in $\progV(c)$ is computed as follows,
