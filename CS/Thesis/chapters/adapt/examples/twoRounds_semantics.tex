\begin{example}[Evaluation Trace for The Example Program of Two Rounds Data Analysis]
    In the follows, we reuse the same example program $\kw{towRounds(k)}$ as Figure~\ref{fig:overview-example}(a),
    ...
    $\trace_0 = [(k, \lin, 2, \bullet)]$,
    the execution trace generated from this example is 
   \\
    $
    \left[\begin{array}{l}
    % \trace_0 \tracecat
