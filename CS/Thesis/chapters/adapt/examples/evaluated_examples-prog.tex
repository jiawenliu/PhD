Below are the example programs in the evaluation table~\ref{tb:adapt-imp}
...
\clabel{ \assign{y}{\query(\chi[1])}}^{2} ; \\
    ...
    \Big(
     ...
     \clabel{\assign{j}{k}}^{5} ;\\
     \eif(\clabel{x > 0}^6, \\
     \qquad \clabel{\assign{y}{\query(\chi(\ln(x) + y))} }^{7},
...
\end{array}
   \]
   The Adaptivity Evaluation Result: $ \progA(\kw{nestWhileMPRV}(k)) = 1 + k + k^2$
   \\
   The Reachability Bound Evaluation Result: \\
