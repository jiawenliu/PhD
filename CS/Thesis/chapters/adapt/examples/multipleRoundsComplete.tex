...
\begin{example}[The Multiple Rounds Analyst Strategy]
    %
    Below is the \emph{multiple rounds analyst strategy}. This is instantiated from the
    \emph{Monitor}, the Algorithm 2 in \cite{RogersRSSTW20}.
    We instantiate this algorithm to trace the possible population shown in a database.
    We monitor the score of each person in the database. 
    \begin{algorithm}
    \footnotesize
    \caption{The \emph{multiple rounds analyst strategy}}
    ...
    \end{algorithm}
    %
    We also have the complete implementation of this strategy in {\tt QWhile} below.
    {\small
    \begin{figure}
    ...
    % \end{subfigure}
    % \vspace{-0.3cm}
    \caption{The labeled program implementing the \emph{multiple rounds analyst strategy}.}
    % \vspace{-0.5cm}
    \label{fig:multiround_complete}
    \end{figure}
    }
    The evaluation result is in the $18^{th}$ line in Table~\ref{tb:adapt-imp}, which shows that our {\THESYSTEM} works well for this complete implementation.
