...
\begin{example}[Linear Regression Algorithm with Gradient Decent Optimization]
    ...
    Our $\THESYSTEM$ works well also over the linear regression algorithm with a gradient descent optimization method.
    \begin{figure}
    \centering
    \begin{subfigure}{0.55\textwidth}
     ...
     \Big(
        \\
     \clabel{\assign{da}{\query(-2 \cdot (\chi[1] - (\chi[0]\cdot a + c)) \cdot (\chi[0]))} }^{4} ; \\
     \clabel{\assign{dc}{\query(-2 \cdot (\chi[1] - (\chi[0]\cdot a + c)))} }^{5} ; \\
     \clabel{\assign{a}{a - \kw{r} \cdot da} }^{6} ; 
     \clabel{\assign{c}{c - \kw{r} \cdot dc} }^{7} ; \\
     ...
     \draw[ -Straight Barb] (1.8, 4.2) arc (220:-100:1);
     \draw[ -Straight Barb] (7.5, 6.5) arc (150:-150:1);
     \draw[ -latex] (6, 6.5) -- (6, 8.5) ;
     \draw[ -Straight Barb] (1.7, 1.) arc (120:-200:1);
     % Value Dependency Edges on Initial Values:
     \draw[ -latex,] (-2, 1) -- (-4.5, 1) ;
     \draw[ -latex,] (-2, 4) -- (-4.5, 4) ;
     %
     \draw[ thick, -latex, densely dotted,] (-1, 1.5) to [out=-220,in=220] (-1, 6.5);
     \draw[ thick, -latex, densely dotted,] (-1, 4.5) to [out=-220,in=220] (-1, 9.5);
     \draw[ thick, -latex, densely dotted,] (1, 6.2) to [out=-60,in=60] (0.5, 1.5) ;
     \draw[ thick, -latex, densely dotted,] (1.2, 9.2) to [out=-50,in=50] (0.5, 4.5);
     \draw[ -latex] (1.8, 7) -- (5.5, 6) ;
     \draw[ -latex] (1.8, 4) -- (5.5, 6) ;
     \draw[ -latex] (1.8, 1) -- (5.5, 6) ;
     \draw[ -latex] (1.8, 10) -- (5.5, 6) ;
     \end{tikzpicture}
     \caption{}
     \end{centering}
     \end{subfigure}
    ...
    % in order to
    aims to
    find a linear relationship, $y = a \cdot x + c$ between a dependent variable $y$
    ...
    $\query(-2 * (\chi[1] - (\chi[0]\cdot a + c)) \cdot (\chi[0]))$ and 
    $\query(-2 * (\chi[1] - (\chi[0]\cdot a + c)))$
    ...
    % Though the different weight functions count the execution times of the different corresponding commands,
    % the counts are expected to be the same, i.e., the loop iterations.
