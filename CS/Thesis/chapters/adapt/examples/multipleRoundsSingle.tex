\begin{example}[Single Adaptivity Round Example]
    ...
    The function $\lambda \trace_0 \sthat \env(\trace_0) k$ counts the execution times of command $\clabel{\assign{p}{\query(\chi[y]+p)} }^{7}$ during execution, which is expected to be equal to the loop iteration numbers, i.e., initial value of input $k$ from the initial trace $\trace_0$.
