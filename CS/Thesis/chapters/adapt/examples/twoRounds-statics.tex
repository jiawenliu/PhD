\paragraph{Example I.}
Below in Figure~\ref{fig:twoRounds-adaptfun}(c) we show again the program and the graph, which is the same walk-through example as Figure~\ref{fig:overview-example}(a).
    % We represent the program and the graph below in Figure~\ref{fig:twoRounds-adaptfun}(c) again for reminding.
    The estimated dependency graph that our static analysis algorithm returns for the program $\kw{twoRounds(k)}$ is in Figure~\ref{fig:twoRounds-adaptfun}(a).
    Vertices and query annotations are the same as the ones in Figure~\ref{fig:twoRounds-adaptfun}(b) and they are simply inferred by scanning the program.
    As we said before, the edges are estimated using control flow and data flow analysis.
    For the $\kw{twoRounds(k)}$ example, every edge in Figure~\ref{fig:twoRounds-adaptfun}(b) is precisely inferred by our combined analysis, this is why Figure~\ref{fig:twoRounds-adaptfun}(c) contains exactly the same edges.
    ...
    For example, consider the vertex $x^{3}$, its weight is $k$ and this provides an upper bound on the values returned by the weight function $\lambda \trace. \rho(\trace)k$ associated with vertex $x^{3}$ in Figure~\ref{fig:twoRounds-adaptfun}(b) for any initial state. 
   ...
   \caption{}
    ...
    \end{subfigure}
   \vspace{-0.4cm}
    \caption{(a) The same running example program $\kw{towRounds(k)}$ as Figure~\ref{fig:overview-example}(a).
   (b) The corresponding semantics-based dependency graph. 
   (c) The estimated dependency graph from $\THESYSTEM$.
   }
   \label{fig:twoRounds-adaptfun}
   \end{figure}
   }
%    \end{example}