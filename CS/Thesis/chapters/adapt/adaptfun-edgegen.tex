...
% This algorithm named Feasible Data-Flow Generation. 
It 
...
\begin{enumerate}
  \item 
  We first perform the \textbf{reaching definition} analysis and compute a set of labeled variables, $\live(l, c)$ for every label $l$ in $c$
  ...
  for every pair of labeled variable in a program $c$.
  ...
  It combines the $\live(l, c)$, $\absG(c)$ and the data flow analysis, which gives more precise approximation on the \emph{data may-dependency} than single analysis in Appendix.
  % in Definition~\ref{def:feasible_flowsto}. 
...
\item The operator  $in(l)$, $out(l)$: $ \ldom \to \mathcal{LV} \cup \{?\}$ for every block in program $c$ is defined as follows,
 \[
 \begin{array}{ll}
    % in(l)   \\  \qquad \triangleq \{ (x, ?) | x^l \in \lvar_c \land  l = \absinit(c) \}  
    in(l)   \triangleq \{ x^{?} | x^l \in \lvar_c \land  l = \absinit(c) \}  
