\highlight{Improved significantly from previous work in terms of complexity,
this operational semantics is defined based on only the event and trace with an efficient 
environment operator as follows.
}
\paragraph{Event}
\highlight{
  In previous semantics design, trace tracking only the annotated query.
...
\]
 ...
 \highlight{
  ...
  Because all evaluation information is stored in the trace in the form of the event,
   and the variable values is recovered from it through $\env$ operator,
  only the trace and program is necessary during the evaluation.
 }
...
% And the result $v$ of evaluating $\expr$ is used to construct a new event $\event = (x, l, v,\bullet)$ and attach to the previous trace. 
...
and boolean expressions $\config{\trace, \bexpr} \barrow v $ in Figure~\ref{fig:expr_eval}.
% \begin{mathpar} 
\begin{figure}
\begin{mathpar}
