...
For every program $c \in \cdom$ and traces $\trace, \trace' \in \ftdom$, if 
$\config{c, \trace} \rightarrow^{*} \config{\clabel{\eskip}^l, \trace'}$,
then there exists a trace $\trace'' \in \ftdom$ with $\trace \tracecat \trace'' = \trace'$
%
$$
\forall \trace, \trace' \in \ftdom, c \in \cdom, l \in \ldom \sthat 
\config{c, \trace} \rightarrow^{*} \config{\clabel{\eskip}^l, \trace'} 
\implies \exists \trace'' \in \ftdom \sthat \trace \tracecat \trace'' = \trace'.
$$
Again, we want to emphasize that in \redd{PART \romannum{1}}, we consider as well-formed only the finite trace when we say $\trace \in \ftdom$, and the terminating execution.
\end{lem}
