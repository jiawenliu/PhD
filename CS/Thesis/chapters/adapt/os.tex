...
The query expression evaluation relation  $\config{\trace, \qexpr} \qarrow \qval$ is defined by the following rules.
...
The normal form $\qval$ characterizes the linear query that is run against a hidden database through an intermediate mechanism.
For example, a query expression $\chi[x] - \chi[x]$ with a trace $\trace = [(x, 0, 1, \bullet)]$
is evaluated into the query value of normal form $\chi[1] - \chi[1]$.
{\small
\begin{mathpar}
\inferrule{ 
...
\begin{defn}[Well-formed Program]
  ...
  under arbitrary initial trace and hidden database. We use $\cdom$ to denote the set of all well-formed program.
  \[
...
\end{defn}
For the events that are generated from query request command, as in Equation~\ref{eq:event_eq}(c),
...
it is possible that two events has different values of query answer in the third position, but they
are equivalent by Definition~\ref{def:query_equal}.
For example, the events $(x, 0, 1, \chi[1])$ and $(x, 0, 2, \chi[1])$ are equivalent by this definition even though they have different query answer assigned to variable $x$.
