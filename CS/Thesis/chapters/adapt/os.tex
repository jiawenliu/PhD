...
This operational semantics is defined based on the event and trace with an environment operator as follows.
...
$ \pi_i : 
\eventset \to \mathcal{V}\cup \mbox{Boolean Expression}  \cup \mathcal{L} \cup \mathcal{VAL} \cup \mathcal{QVAL} \cup\{\bullet\} $.
...
The environment is simply an operator, $\env$ where $\env(\trace) x$ fetches the latest value assigned to $x$ in the trace $\trace$.
...
In these evaluation rules, $v_1 \oplus_a v_2$ denotes the corresponding mathematic computation of two values and the same for $\elog v$ and $\esign v$.
