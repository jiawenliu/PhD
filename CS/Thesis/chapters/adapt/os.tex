...
We use trace-based operational semantics to track the history of program execution.
...
we need to be able to identify whether two queries are equivalent or not, so that when we change the result of one query, 
...
we cannot define their equivalence based on comparing the query answer.
In this sense, we 
define the equivalence of query expression by quantifying over all possible values returned from the database on a certain form of query value, formally as follows.
...
\end{defn}
%
Then, we define the equivalence of two events generated from a query request by comparing the query value rather than the query answers.
