...
Based on the event recording all the program's evaluations for expressions, 
 ...
 A \emph{trace}
 $\trace$ is a list of \emph{events} generated when executing specific commands. We denote by $\mathcal{T}$ the set of all finite traces and we will use list notation for traces, where $[]$ is the empty trace, the operator $\traceadd$ combines an event and a trace in a new event, 
...
%
\\
We denote by $\mathcal{T}$ the set of all finite traces.
If a program $c$ has some non-terminating behavior, it will produce an infinite trace from $\mathcal{T} \cup \{\infty\}$. 
...
operator $\llabel : \mathcal{T} \to \mathcal{V} \to \mathbb{N} \cup \{ \bot\}$,
...
Formally, the environment operator, $\env : {\mathcal{T}}  \to \mathcal{V} \to \mathcal{VAL} \cup \{\bot\}$ maps a trace and a variable to the latest value assigned to this variable on the trace, as defined below.
\[
\begin{array}{lll}
...
\inferrule{ 
 ...
 then the program's execution terminates and produces a finite execution trace $\trace \in \mathcal{T}$. 
...
\begin{defn}[Non-terminating and Infinite Trace]
  ...
  Given a program $c$ and an initial trace $\trace \in \mathcal{T}_0(c)$,
  when $c$ executes with $\trace$,  we define the execution of $c$ under $\trace$ is non-terminating and produces an infinite trace $\trace' \in \mathcal{T} \cup \{\infty\}$, as 
  ...
  \[
    ...
    \Big\{
      ...
      c\in \cdom, \trace \in \mathcal{T}_0(c),
      \trace' \in \mathcal{T} \cup \{\infty\} 
      \land \exists \trace_0 \in \mathcal{T}, c_0 \in \cdom \sthat 
      \config{c, \trace} \to \config{c_0, \trace_0}
      \\ \qquad \qquad \qquad 
      \land \forall i \in \mathbb{N}, \exists \trace_i, \trace_{i + 1} \in \mathcal{T}, \trace'' \in \mathcal{T} \cup \{\infty\}, c_i, c_{i + 1} \in \cdom \sthat 
  ...
  \]
\end{defn}
In the rest of {\redd{PART \romannum{1}} we consider as well-formed only the terminating execution and finite trace denote by $\trace \in \mathcal{T}$.
We will extend to the non-terminating programs in {\redd{PART \romannum{2}} and use $\ftdom$ and $\inftdom$ to distinguish the universe of finite trace and infinite trace.
...
%
$$
\forall \trace, \trace' \in \mathcal{T}, c \in \cdom, l \in \ldom \sthat 
\config{c, \trace} \rightarrow^{*} \config{\clabel{\eskip}^l, \trace'} 
\implies \exists \trace'' \in \mathcal{T} \sthat \trace \tracecat \trace'' = \trace'.
$$
Again, we want to emphasize that in \redd{PART \romannum{1}}, we consider as well-formed only the finite trace when we say $\trace \in \mathcal{T}$, and the terminating execution.
\end{lem}
