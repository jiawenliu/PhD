...
Dynamic dependency graph have been used in many other domains. For example,~\citet{NagarJ18} use dynamic dependency graphs to find serializability violations. \citet{HammerGK06} proposes similar ways of constructing different kinds of program slices based on the algorithm presented in~\cite{ferrante1987program}, by choose different program dependency graph.
...
is inspired by the method in~\citet{Cousot19a}, where the dependency relation is also identified by looking into the differences on two execution traces. 
...
The idea of statically estimating a sound upper bound for the adaptivity from the semantics is indirectly inspired from prior work on cost analysis via effect systems in~\cite{CicekBG0H17, RadicekBG0Z18, QuG019}. The idea of defining adaptivity using data flow is inspired by the work of graded 
Hoare logic~\cite{GaboardiKOS21}, which reasons about data flows as a resource. 
%
