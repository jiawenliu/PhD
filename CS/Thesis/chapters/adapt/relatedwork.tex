...
dependency~\cite{bilardi1996framework, CytronFRWZ91, PollockS89, BurkeR90}.    
One related work  
\cite{AustinS92} presents a methodology to construct a dynamic dependency graph (DDG) based on the dynamic execution of a program in an imperative language, where edges represent dependency between instructions. Data dependency, control dependency, storage dependency, and resource dependency between instructions are all considered. My execution-based dependency graph only needs data dependency and control dependency between variable assignment results. 
%

DDGs have been used in many other domains. \citet{NagarJ18} use DDGs to find serializability violations. \citet{HammerGK06} proposes similar ways of constructing different kinds of program slices based on~\cite{ferrante1987program}, by choose different program dependency graph.
...
However, Cousot excludes timing channels~\cite{SabelfeldM03} and empty observation, which are also not considered as a form of dependency in traditional dependency analysis \citet{DenningD77}.
...
effect systems, control-flow analysis, and data-flow analysis~\cite{BurkeR90}. 
The idea of statically estimating a sound upper bound for the adaptivity from the semantics is indirectly inspired from prior work on cost analysis via effect systems~\cite{CicekBG0H17, RadicekBG0Z18, QuG019}. The idea of defining adaptivity using data flow is inspired by the work of graded 
...
Another work \citet{ArltPBSBM12} constructs a weighted event-dependency graph, capturing data dependencies between events by analyzing bytecode. 
