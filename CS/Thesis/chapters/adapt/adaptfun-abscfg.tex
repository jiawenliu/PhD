...
\[ 
  \absV(c) = \left\{ l ~\vert~ \exists x \in \vardom \sthat x^l \in \lvar(c)  \right\}\cup\{{\lex}\}
\]
...
The \emph{Symbolic Constant} domain $\constdom = \mathbb{N} \cup \inpvar \cup \left\{ \infty, Q_m \right\}$
is the set of all natural numbers with $\infty$, the input variables of all programs (i.e., $\inpvar \subseteq \vardom$), and a symbol $Q_m$ representing the abstract value of a query request.
...
\highlight{Computation Steps are summarized below. The computation below is a context free computation skeleton overall the variable and value domain. In a specific computation with given program $c$, these domains will associate to the program $c$.
}
...
% \\
\begin{defn}[Symbolic Expression Domain ($\mathcal{A}_{S}$)]
  \label{def:symbolic_expr_domain}
  $\mathcal{A}_{S}$ is the set of all the arithmetic expressions 
...
For every event $\event$ with label $l$, if it is in an execution trace $\trace$ of the program $c$, 
   ...
   \[
     \begin{array}{l}
    \forall c \in \cdom, \vtrace_0 \in \mathcal{T}_0(c), \trace \in \mathcal{T} ,  l, l' \in \absV(c),  \event = (\_, l, \_, \_) \in \eventset \sthat
...
\[
  \begin{array}{l}
    \forall c \in \cdom, \vtrace_0 \in \mathcal{T}_0(c), \trace \in \mathcal{T} , l, l' \in \absV(c), \event = (\_, l, \_) \in \eventset^{\asn} \sthat
\config{{c}, \trace_0} \to^{*} \config{\clabel{\eskip}^{l'}, \trace_0 \tracecat \vtrace} 
\land \event \in \trace 
\\
\qquad \implies \exists! \absevent = (l, \_, \_) \in (\ldom\times \dcdom^{\top} \times \ldom) \sthat 
