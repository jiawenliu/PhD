...
Below we present in detail how to compute them and construct this graph.
...
\[ 
  \absV(c) = \bigcup\limits_{x^l \in \lvar(c) }\left\{ l  \right\}\cup\{{\lex}\}
\]
...
\begin{enumerate}
  ...
  Each edge is an \emph{abstract event}, which is a pair of initial and finial state.
\end{enumerate}
%
...
Below we compute the constraints.
It is a context free computation skeleton overall the variable and value domain. In a specific computation with given program $c$, these domains will associate to the program $c$.
...
\begin{defn}[Constraint Computation]
  ...
  \footnote{We omit the input $c$ in the rest of \redd{PART \romannum{1}} if the context is clear. }
...
% \\
\begin{defn}[Universe of The Symbolic Expression ($\mathcal{A}_{S}$)]
  \label{def:symbolic_expr_domain}
  $\mathcal{A}_{S}$ is the universe of all the arithmetic expressions 
over $\constdom$, $\mathcal{A}_{S} \subseteq \mathcal{A}$
\end{defn}
...
%  It is 
 The set of abstract events for a program $c$ is generated by computing the initial state and finial state interactively and recursively.
%  \begin{itemize}
...
For every event $\event$ with label $l$, if it is in an execution trace $\trace$ of the program $c$, 
   ...
   \[
     \begin{array}{l}
    \forall c \in \cdom, \vtrace_0 \in \ftdom_0(c), \trace \in \ftdom ,  l, l' \in \absV(c),  \event = (\_, l, \_, \_) \in \eventset \sthat
...
\[
  \begin{array}{l}
    \forall c \in \cdom, \vtrace_0 \in \ftdom_0(c), \trace \in \ftdom , l, l' \in \absV(c), \event = (\_, l, \_) \in \eventset^{\asn} \sthat
\config{{c}, \trace_0} \to^{*} \config{\clabel{\eskip}^{l'}, \trace_0 \tracecat \vtrace} 
\land \event \in \trace 
\\
\qquad \implies \exists! \absevent = (l, \_, \_) \in (\ldom\times \dcdom^{\top} \times \ldom) \sthat 
...
\paragraph*{Example}
%
\input{chapters/adapt/examples/abscfg_tworound}