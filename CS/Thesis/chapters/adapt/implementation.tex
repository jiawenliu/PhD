...
\input{chapters/adapt/tables/eval-table}
...
The next eleven programs from Table~\ref{tb:adapt-imp} column 7 to 18 are handcrafted programs based on the code pieces extracted from C library. They all have small size but complex structures in order to test the programs under different situations including
data, control dependency,
the multiple paths nested loop with related counters, etc.
The algorithm computes the tight bound for examples from line:6, $\kw{ifCD()}$ to line:{$14$}, $\kw{loop2MPRV(k)}$
...
$\THESYSTEM$.
For the $17^{th}$ example ($\kw{loop2RC}$), {\THESYSTEM} gives tight bound while the alternative implementation gives loose bound, so we keep both implementations.
...
\input{chapters/adapt/tables/eval-table-alternatives}
The programs we evaluated in the Appendix~\ref{apdx:evaluated_examples} and Appendix~\ref{apdx:evaluated_codes}.

%  \subsection{More Discussions on The Evaluated Examples}  
 Below we discuss in detail the real-world adaptive data analyses that we evaluated.
 The first one is the two rounds adaptive data analysis which we present as the walk through example.
 The second one is the complete multiple rounds adaptive data analysis of the simplified version that we discussed in Example~\ref{ex:multipleRounds}.

 \paragraph{The Two Rounds Adaptive Data Analysis Algorithm, $\kw{tRComplete}$} 
 \input{chapters/adapt/examples/twoRoundsComplete}
 %
 \paragraph{The Multiple Rounds Adaptive Data Analysis Algorithm, $\kw{mRComplete}$} 
 \input{chapters/adapt/examples/multipleRoundsComplete}
 %
