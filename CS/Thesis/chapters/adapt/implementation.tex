...
For the forth program $\kw{multiRoundsO(k)}$, $\THESYSTEM$ outputs an over-approximated upper bound $1 + 2*k$ 
...
The next eleven programs from Table~\ref{tb:adapt-imp} row 7 to 18 are handcrafted programs based on the code pieces extracted from C library. They all have small size but complex structures in order to test the programs under different situations including
