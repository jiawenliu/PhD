...
An extended arithmetic expression can be a constant $n$ denoting natural number or the $\infty$ symbol, a variable $x$ from some countable set $\vardom$, binary operation $\oplus_a$ such as addition, product, subtraction, etc, over arithmetic expressions, and also $\emax$, $\emin$, $\elog$ and $\esign$ operation. 
%
...
When a query expression is encapsulated by the symbol $\query$, it represents a linear query request.
A linear query compute the average value of certain query expression over each row of the hidden database.
Through mechanisms that intermediate between programmers and database, the answer of a linear query is computed and return.
For example, if the mechanism computes the empirical answer of the query request
$ \query(\chi[\aexpr])$, then it returns the average value at index $\aexpr$ over each row of the database as follows,
 \[
...
The label has to be added by programmer start from $0$
We use the label $\lin$ as the labels of the variables in an initial trace if a program has input variables,
...
\paragraph{Free Variables and Input Variables}
  ...
  We compute the input variable by taking sup of the free variables in every expression
  and subtract the variables that are assigned.
...
\begin{lem}[Uniqueness of the Labeled Variables]
  ...
  \[
    ...
    \]
\end{lem}
We guarantee that, even there are two labeled variables in $\lvar(c)$ have the same variable name,
such that $x^i$ and $x^j$, we still have $x^i \neq x^j$ through $i \neq j$.
