...
I give a new adaptivity formalization model by reasoning program's semantics in Section~\ref{sec:adapt-exe}
summarized as follows.
\begin{enumerate}
 \item For any variables $x^i, y^j$ in a program $c$, 
 we first define a \emph{variable may-dependency} relation between them semantically as
 \[
   \vardep(x^i, y^j, c).
  \]
 Then we reason the \emph{dependency relation} between queries.
 %
