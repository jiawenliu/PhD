...
\paragraph{Limitations of Existing Program Semantics and Execution-Based Analysis Techniques}

\begin{itemize}
 \item \textbf{Limitations in Program Semantics and Execution-Based Formalization}
 \\
...
\begin{enumerate}
\item The state-of-art data dependency relation model don't
consider both control influence and value influence. 
However, both of these
two dependencies are counted as the query dependency 
in the adaptive data analysis programs.
% require
\item In the existing data dependency analysis, there isn't any definition taking into account
the dependency depth. However, this dependency depth is the key quantity in formalizing the intuitive \emph{adaptivity} as a \emph{quantitative reachability} program property.
\item The \emph{adaptivity} doesn't accumulate linearly through the dependency relation and 
dependency depth. There is only very limited works related to this non-monotonic program quantitative property~\cite{AlbertFR15} analysis.
This requires us to design new semantics formalization model in order to define \emph{adaptivity} concept precisely.
\end{enumerate}
