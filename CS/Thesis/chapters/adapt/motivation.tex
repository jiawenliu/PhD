...
data dependency relation, and defines the \emph{quantitative} as the reachability-bound, and then defines the
\emph{adaptivity} over it.
We summarize them into three sub-research goals as follows.
...
\item[sub-goal 2.] From the \emph{quantitative} perspective, the state-of-art data dependency relation formalization, there isn't any technique taking into account
...
\item[sub-goal 3.] The intuitive \emph{adaptivity} doesn't accumulate linearly through the dependency relation and 
...
We summarize them as three sub-goals below. Each of them aims to resolve a fact introduced in the \textbf{Adaptivity Estimation} challenge in Section~\ref{sec:adapt-motivation}.

\begin{enumerate}
\item[sub-goal 1.] From the \emph{reachability} perspective, existing static dependency analysis techniques do not consider both control influence and value influence.
Our goal is to design new a data flow analysis algorithm. This new algorithm need to estimate whether a query may depend on the other queries. 
It also needs to be able to capture the query dependency soundly
under the control influence.
\item[sub-goal 2.] From the \emph{quantitative} perspective, existing static analysis techniques in the complexity or resource cost estimation areas are not focusing on the queries or data in the program. 
...
%  techniques.
\item[sub-goal 3.] To estimate the \emph{adaptivity} in the end for programs,
the last research goal is design new algorithm to compute the longest \emph{finite walk} over the dependency graph.
\end{enumerate}
