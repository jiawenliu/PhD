...
In order to analyze this \emph{adaptivity} as a program property, there are mainly three challenges.
...
% \begin{enumerate}
%  \item
\subsubsection{Adaptive Data Analysis Formalization.}
...
% to define this intuitive \emph{adaptivity} precisely as a \emph{quantitative reachability} program property, 
% we give a new adaptivity formalization model by reasoning program's semantics in Section~\ref{sec:adapt-exe}
% summarized as follows.
% \begin{enumerate}
%  \item For any pair variables $x^i, y^j$ in a program $c$, 
%  we first define a \emph{variable may-dependency} relation between them by looking into their semantics as
%  \[
%    \vardep(x^i, y^j, c).
%   \]
%  Then we reason the \emph{dependency relation} between queries through this relation and an extra annotation on variables indicating whether it is a query request.
%  %
...
%  \item The second step in Section~\ref{sec:dynamic-graph} constructs a \emph{semantics-based dependency graph} combining the \emph{dependency relation}
%   with a \emph{quantitative relation} as weight. This graph is denoted as $\traceG(c)$ for the program $c$ has an edge set $\traceE(c)$,
%   vertex set $\traceV(c)$, weights $\traceW(c)$ and an annotation set $\traceF(c)$.
...
%  \item In the last step we define the intuitive \emph{adaptivity}
%  according to the two analysis results above, specifically the \emph{variable may-dependency} relation and \emph{dependency quantity} (vertex weight).
%  This step 
