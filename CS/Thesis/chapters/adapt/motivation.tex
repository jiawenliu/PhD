...
we summarize them into three sub-technical motivation as follows.
\begin{enumerate}
\item[sub-motivation 1.]
...
\item[sub-motivation 2.] From the \emph{quantitative} perspective, the state-of-art data dependency relation formalization, there isn't any technique taking into account
...
\item[sub-motivation 3.] The intuitive \emph{adaptivity} doesn't accumulate linearly through the dependency relation and 
...
\begin{enumerate}
\item[sub-motivation 1.] From the \emph{reachability} perspective, either static dependency analysis techniques or the control flow analysis consider only
...
\item[sub-motivation 2.] From the \emph{quantitative} perspective, existing static analysis techniques in the complexity or resource cost estimation areas are not focusing on the queries or data in the program. 
...
%  techniques.
\item[sub-motivation 3.] To estimate the \emph{adaptivity} in the end for programs, we want to reuse some path search algorithm
