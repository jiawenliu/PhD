In the previous works, the adaptivity is estimated
...
It first re-writes the program from the loop language into
a Static Single Assignment (SSA) version of loop language, and then estimates the adaptivity through a program analysis algorithm.
...
the same variable, they re-write the program into the SSA form.
...
%
~|~ {{\eifvar(\bar{\ssa{x}}, \bar{\ssa{x}}')}} ~|~ [\eskip]^{l} 
\\ &&&
~|~
 \eloop ~ [{\ssa{\aexpr}}]^{l}, {n}, [\bar{\ssa{x}}, \bar{\ssa{x_1}}, \bar{\ssa{x_2}}] ~ \edo ~ {\ssa{c}}
 \\ &&& ~|~  \ssa{c};\ssa{c} ~|~ \eif([\ssa{\bexpr}]^{l}, ([\bar{\ssa{x}}, \bar{\ssa{x_1}}, \bar{\ssa{x_2}}] , [\bar{\ssa{y}}, \bar{\ssa{y_1}}, \bar{\ssa{y_2}}],[\bar{\ssa{z}}, \bar{\ssa{z_1}}, \bar{\ssa{z_2}}] ) , \ssa{c}, \ssa{c}) 
\end{array}
\]
In the SSA form, the if command contains an extra part,
 $([\bar{\ssa{x}}, \bar{\ssa{x_1}}, \bar{\ssa{x_2}}]$, $
[\bar{\ssa{y}}, \bar{\ssa{y_1}}, \bar{\ssa{y_2}}]$, $[\bar{\ssa{z}}, \bar{\ssa{z_1}}, \bar{\ssa{z_2}}] )$,
to track the dependency of newly assigned variables in both branches
% ($[\bar{\ssa{x}}, \bar{\ssa{x_1}}, \bar{\ssa{x_2}}]$),
$[\bar{\ssa{y}}, \bar{\ssa{y_1}}, \bar{\ssa{y_2}}]$ and  $[\bar{\ssa{z}}, \bar{\ssa{z_1}}, \bar{\ssa{z_2}}] $. 
...
The SSA form of the loop command also has an extra part $ [\bar{\ssa{x}}, \bar{\ssa{x_1}}, \bar{\ssa{x_2}}]$ 
to track the variables in the loop body.

The operational semantics rules
% with all the operations defined under the loop language introduced 
in Section~\ref{sec:prework-language} are all translated into the corresponding SSA form as well.
Based on the transformed language, they also redefine the formal adaptivity for the transformed program equivalent to the
loop language-based definition. The complete definition is in the thesis by~\cite{weihao22}.
%
\subsection{Program Analysis Algorithm}
This part summarizes 
the adaptivity analysis algorithm that performs on the SSA version of the loop language.
This algorithm is
...
% \\
 ...
 This algorithm adds all the SSA version program variables to the global variable list $G$.
...
\end{mathpar}
 ...
 \caption{The key rule of the variable estimation algorithm. }
...
Some key rules are selected in Figure~\ref{fig:prework-static_alg2}.
...
\\
They use a standard longest-path search algorithm to compute the adaptivity bound over the variable-based dependency graph.
\end{enumerate}


\subsection{Limitations}
Limitations of this adaptivity estimation algorithm is summarized from the same three aspects as follows.

\emph{Efficiency.}
 \begin{enumerate}
 \item \textbf{SSA Transformation}
 ...
 However, this rewriting is inefficient and unnecessary.
 ...
 \item \textbf{Variable Estimation Algorithm}
 According to the rules \textbf{ag-loop} in Figure~\ref{fig:prework-static_alg1},
 and rule \textbf{ad-loop} in Figure~\ref{fig:prework-static_alg2}, the algorithm needs to
 unfold every iteration of the while loop and create new annotated variables for every iteration.
 This operation increased the complexity of the program analysis by exponential factors, which is inefficient.

 \item \textbf{Path Search and Adaptivity Estimation}
 Based on the unfolded loops and exponential numbers of the annotated variables, the graph
 generated over these variables are exponentially huge as well.
 However, to estimate the adaptivity over this graph, we need to search for the longest weight path over it.
 The exponential size of the graph increases the burden of the path search algorithm significantly.
 \end{enumerate}
 %
 \emph{Expressiveness.}
 The variable estimation algorithm is unable to analyze the programs with
 ...
 In order to guarantee the termination of the analysis, they have to limit the loop with a constant number of loop iterations.
 ...
 It over-approximates in cases where there isn't semantic dependency between variables even though the variables
 are explicitly used in the query request.
 This is also because the algorithms that are used in each step are simply the standard algorithms that do not naturally
 compatible with each other.
 So the straightforward composition is not able to estimate
 the adaptivity property accurately when considering it as
