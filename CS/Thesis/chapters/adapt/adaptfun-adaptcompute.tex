...
Different from the weight on the semantics-based dependency graph, the weight on the estimated graph are symbolic expression. This symbolic expression cannot be used directly to construct a finite walk
by definition~\ref{def:finitewalk}. In this sense,
% to estimate the finite walk
we first introduction the path on the estimated dependency graph as follows to estimate a finite walk.
\begin{defn}[Path]
...
\begin{defn}[Query Length Estimation]
  ...
  $(v_1, \ldots, v_{n})$,
  the estimated query length of $p$, $\hat{\qlen}(p)$ is computed as
  \[
...
In this sense, the computation by Definition~\ref{def:prog_adapt} have to
over-approximate the adaptivity in a large scale in order to address the non-terminating problem.
...
\input{chapters/adapt/examples/kRoundsWhileSim_alg}

To solve this non-termination challenge, we switch to another walk finding approach:
finding the longest path in the estimated dependency graph via depth first search and then use this path as the longest walk.
...
\begin{algorithm}
  ...
  % \STATE \qquad \qquad \qquad \highlight{$\kw{r[v] + = \weights(v)\times \qflag(v) $}; 
...
%
$\pathsearch(c, \progG(c))$ algorithm in Algorithm~\ref{alg:adapt} arranges the estimated dependency graph $\progG(c)$ into SCCs ($\kw{\sccgraph_1}, \cdots, \kw{\sccgraph_n}$) and obtains the adaptivity local bound of each SCC from $\kw{\pathsearch_{SCC}(c, \sccgraph_i)}$ algorithm in Algorithm~\ref{alg:adaptscc}.
...
We discuss in detail as below this adaptivity computation algorithm.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{algorithm}
  ...
  \STATE {\bf init} 
    ...
    \\
    \STATE Find all strongly connected components in $G$: $\kw{\sccgraph_1}, \cdots, \kw{\sccgraph_n}, 0 \leq n \leq |\vertxs|$, 
    ...
    %
    % it 
    At Line:3, this algorithm first finds all the SCCs of $\progG(c)$, $\kw{\sccgraph_1}, \cdots, \kw{\sccgraph_n}$
    ...
    Then, 
    it computes the adaptivity local bound on every $\kw{\sccgraph_i}$
    ...
    $\kw{\sccgraph_1}, \cdots, \kw{\sccgraph_n}$ are all the vertices and the adaptivity local bounds are their weights.
...
alone the path when the traverse arrives vertex $v$ by the operation 
$\kw{flowcapacity[v] \times querynum[v]}$.
...
\input{chapters/adapt/examples/nestedWhile_alg}
    %
