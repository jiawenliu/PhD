...
Following the same procedure during defining the adaptivity in Definition~\ref{def:qlen}, we define
...
{
Given a program ${c}$, we first compute its \emph{estimated dependency graph} 
$\progG({c})$, then we compute adaptivity
%
 for $c$ as 
\[
\progA({c})
...
\begin{thm}[Soundness of {\THESYSTEM}]
  ...
  its estimated adaptivity is a sound upper bound of its adaptivity.
   \[
   \forall \trace_0 \in \ftdom_{0}(c), v \in \mathbb{N}^{\infty} \sthat
...
However, the computation in Definition~\ref{def:prog_adapt} is non-terminating given the weight as symbolic.
