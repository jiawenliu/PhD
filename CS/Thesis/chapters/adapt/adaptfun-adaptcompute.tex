...
%
we compute a sound upper bound on $c$'s adaptivity in this section in Definition~\ref{def:prog_adapt}
and Algorithm~\ref{alg:adapt}.
%

Different from the weight on the semantics-based dependency graph, the weight on the estimated graph are symbolic expression. This symbolic expression cannot be used to construct a finite walk
by definition~\ref{def:finitewalk}. In this sense, to estimate the finite walk
we first introduction the path on the estimated dependency graph as follows.
\begin{defn}[Path]
...
\begin{defn}[Query Length Computation]
  ...
  $(v_1, \ldots, v_{n})$,
  the query length of $p$, $\hat{\qlen}(p)$ is computed as
  \[
...
In this sense, the computation in Definition~\ref{def:prog_adapt} is non-terminating given the weight as symbolic.
...
\begin{thm}[Soundness of $\pathsearch$]
    ...
    \[ 
      \forall \trace_0 \in \ftdom_{0}(c), v \in (\mathbb{N}\cup \{\infty\}) \sthat
    \config{\pathsearch(c), \trace_0} \earrow v \implies A(c)(\trace_0) \leq c.
    \]
\end{thm}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INTRODUCTION OF ADAPTIVITY COMPUTATION ALGORITHM 1: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph*{Adaptivity Computation for An Strong Connected Component of $\progG(c)$.}
\begin{algorithm}
  \caption{
  {Adaptivity Computation on An SCC ({$\kw{\pathsearch_{scc}(c, SCC_i)}$})}
  \label{alg:adaptscc}
  }
  \begin{algorithmic}[1]
    \REQUIRE 
    The program $c$, 
    \\ \qquad
    An strong connected component of $\progG(c)$: 
    $ \kw{SCC_i} = (\vertxs_i, \edges_i, \weights_i, \qflag_i)$
