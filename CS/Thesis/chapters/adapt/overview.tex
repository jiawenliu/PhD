...
adaptive data analysis has three major components
% is composed of three parts 
as shown in Figure~\ref{fig:adaptfun-structure}.
The first component on the top of Figure~\ref{fig:adaptfun-structure} is the {\tt QWhile} language designed for formalizing the 
adaptive data analysis. 
Building on this, 
we formally define the intuitive \emph{adaptivity} as in the second component at the middle of Figure~\ref{fig:adaptfun-structure}, the ``Adaptivity Formalization'',
and then design a static program analysis algorithm for approximating the upper bound of the 
adaptivity as shown
in the block at the bottom of Figure~\ref{fig:adaptfun-structure}, the ``Adaptivity Analysis''.
\begin{figure}
 \centering 
...
\input{chapters/adapt/examples/twoRounds-overview}
...
The implementation of this data analysis in the language of {\THESYSTEM} is presented in Figure~\ref{fig:overview-example}(a).
...
We use $U$ to denote the codomain of queries and it could be $[-1,1]$, $[0,1]$ or $[-R,+R]$, for some $R$ we consider. This function characterizes the linear query we are interested in running. Indeed, as we discussed in the previous section, linear queries compute the empirical mean of a function on rows --- we use $\chi$ to abstract a possible row in the database.
 ...
 In Figure~\ref{fig:overview-example}(a), the queries inside the while loop correspond to the first phase of the data analysis and compute an approximation of 
...
However, capturing this concept formally is surprisingly difficult. The difficulty comes from the fact that a query can depend on the result of another query in multiple ways, by means of data flow or control flow.

\subsection{Adaptivity Definition}
\label{sec:adaptivity-informal}
The central property we are after in this work is the \emph{adaptivity of a program},
which corresponds to the second component, the ``Adaptivity Formalization'' block at the middle of Figure~\ref{fig:adaptfun-structure}.
As shown in the flow-chart in this block, we define formally this notion in three steps, which we will describe in detail in Section~\ref{sec:adapt-exe}. First, we define a notion of dependency, or better \emph{may-dependency}, between variables. To do this we take inspiration from previous works on dependency analysis and information flow control and we say that a variable \emph{may depend} on another one if changing the execution of the latter can affect the execution of the former. 
We can see in Figure~\ref{fig:overview-example}(a) that the value of the variable $l$, which corresponds to the result of the execution of the query in the second phase (in the command with label 6), is affected by the value of the variable $x$, which corresponds to the result of the execution of the query at line 3 in the first phase, via the variable $a$.
To formally define this notion of dependency, as in information flow control, we use the execution history of programs recorded by trace semantics (see Definition~\ref{def:var_dep}).
...
$\lambda \trace. \env(\trace) k$ since the corresponding assignment will be executed a number of times that correspond to the value of $k$ in the initial state $\trace$, and $\env$ is the operator reading the value of $k$ from $\trace$.
...
Looking again at Figure~\ref{fig:overview-example}(b), and assuming that $\tau(k) \geq 1$, we can see that the 
walk along the dashed arrows, $l^{6} \to a^5 \to x^3 $ has two vertices with query annotation $1$, and we cannot find another walk having more than $2$ vertices with query annotation $1$. So the adaptivity of the program in Figure~\ref{fig:overview-example}(a) with respect to $\tau$ is $2$. If we consider an initial state $\tau$ such that $\tau(k)=0$ we have that the adaptivity with respect to $\tau$ is instead $1$. 
...
This program analysis framework corresponds to the ``Adaptivity Analysis'' block at the bottom of Figure~\ref{fig:adaptfun-structure}.
The detailed structure of {\THESYSTEM} is illustrated in Figrue~\ref{fig:adaptfun-overview}, which reflects in part the definition of adaptivity we discussed in the previous section. Specifically, {\THESYSTEM} is composed of two algorithms (the ones in dashed boxes in the figure), one for building a dependency graph, which we call \emph{estimated dependency graph}, and the other to estimate the adaptivity from this graph. 
...
The second algorithm estimates the longest walk which respects the weights and which visits as many query nodes as possible. 
The two algorithms together give us an upper bound on the program's \emph{adaptivity}.
...
We show in Figure~\ref{fig:overview-example}(c) the estimated dependency graph that our static analysis algorithm returns for the program $\kw{twoRounds(k)}$ in Figure~\ref{fig:overview-example}(a).
Vertices and query annotations are the same as the ones in Figure~\ref{fig:overview-example}(b) and they are simply inferred by scanning the program.
As we said before, the edges are estimated using control flow and data flow analysis.
For the $\kw{twoRounds(k)}$ example, every edge in Figure~\ref{fig:overview-example}(b) is precisely inferred by our combined analysis, this is why Figure~\ref{fig:overview-example}(c) contains exactly the same edges.
The weight of every vertex is computed using a reachability-bound estimation algorithm that outputs a symbolic expression over the input variables, in the example only $k$, representing an upper bound on the number of times each assignment is executed.
For example, consider the vertex $x^{3}$, its weight is $k$ and this provides an upper bound on the values returned by the weight function $\lambda \trace. \rho(\trace)k$ associated with vertex $x^{3}$ in Figure~\ref{fig:overview-example}(b) for any initial state. 
