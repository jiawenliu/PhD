...
The first block on the top is the {\tt Query While} language designed for formalizing the 
adaptive data analysis. 
Building on this, 
we formally define the intuitive \emph{adaptivity} as in the second block, the ``Adaptivity Formalization'' block,
and then design a static program analysis algorithm for approximating the upper bound of the 
adaptivity as 
in the block in the bottom, the ``Adaptivity Analysis'' block.
...
which corresponds to the second block, the ``Adaptivity Formalization'' block in Figure~\ref{fig:adaptfun-structure}.
We define formally this notion in three steps, which we will describe in details in Section~\ref{sec:adapt-exe}. First, we define a notion of dependency, or better \emph{may-dependency}, between variables. To do this we take inspiration from previous works on dependency analysis and information flow control and we say that a variable \emph{may depend} on another one if changing the execution of the latter can affect the execution of the former. 
...
This program analysis framework corresponds to the last block, in Figure~\ref{fig:adaptfun-structure}.
