...
Given a program $c$ with its semantics-based dependency graph $\traceG({c}) = (\traceV, \traceE, \traceW, \traceF)$ of a program $c$, a \emph{walk} $k:\ftdom_0(c)\to \mathbb{N}$ on $\traceG({c})$ is a function that maps an initial trace $\trace_0$ to a sequence of edges $(e_1 \ldots e_{n - 1})$ 
for which there is a sequence of vertices $(v_1, \ldots, v_{n})$ satisfying
\begin{itemize}
