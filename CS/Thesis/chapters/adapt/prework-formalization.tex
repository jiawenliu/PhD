The previous works formalized the adaptivity through a query-based dependency graph.
...
Dependency graph in previous work is over annotated queries.
To construct the edges on this graph, they first
define a query \emph{May-dependency} relation over all queries. Below is a summary of this definition from the
works by~\cite{weihao22}.
The notations $\in_q$ and $\not\in_q$ represent that a query belongs to the trace or not respectively.
...
below is their formal definition of the query-based dependency graph. 
For every execution of a program $c$ starting with a certain memory and while map
% configurations, 
they construct a query-based dependency graph as below.
% graph definition
\begin{defn}[Query-based Dependency Graph]
Given a program $c$, a database $D$, a starting memory $m$, an initial loop map $w$, the query-based dependency graph $G(c,D,m,w)$ has two components, $(V, E)$ defined as: \\
...
% To summarize, for every execution of a program $c$ starting with different configurations, 
% they can construct a corresponding dependency graph. 

\subsection{Adaptivity Formalization}
Below is the definition of adaptivity from works by~\cite{weihao22}, by means of the query-based dependency graph. 
\begin{defn}[Adaptivity in {loop} language]
Given a program $c$, and a memory $m$, a database $D$, a starting loop map $w$, the adaptivity of the dependency graph $G(c, D,m,w) = (V, E)$ is the length of the longest path in this graph. We denote the path from $q({v_q})^{(l,w)}$ to $q({v_q}')^{(l',w')}$ as $p(q({v_q})^{(l,w)}, q({v_q}')^{(l',w')} )$. The adaptivity is denoted as $A(c, D, m, w)$.
%
$$A(c, D, m, w) = \max\limits_{q({v_q})^{(l,w)},q({v_q}')^{(l',w')} \in V } |p(q({v_q})^{(l,w)}, q({v_q}')^{(l',w')} )| $$
\end{defn}

\subsection{Limitations}
Limited by the language design and the dependency graph construction,
this adaptivity definition has the following weaknesses.

\emph{Expressiveness.}
Because the language model only support the program with while loop with constant number of iteration times.
%  The definition isn't general enough to give the formal adaptivity for the adaptive data analysis programs with
%  while loops.
...
% This results in the expressiveness limitations in this adaptivity definition as well.
In this sense, this definition does not general enough to capture the adaptivity of the program with loop whose iteration times dependent on the program state. 
%  \\
 ...
 \}\}
    \end{array}
 \] 
 the number of iterations cannot be evaluated to a natural number in advance of entering this loop for the same reason. 
 The iteration number is only
 able to be decided during executing this while loop body.
 While this adaptivity definition cannot capture this program though the adaptivity in this program is 4, which is straightforward to observe.

 \emph{Accuracy.}
 The inaccuracy of this \emph{adaptivity} definition is caused by both their language design and their graph construction.
% \\
...
% \\
% The other reason for this inaccuracy comes from their graph definition.
 ...
 \emph{Complexity.}
 This definition requires the full unfolding of every iteration for the while loop.
 The unfolding procedure generates many unnecessary and duplicate nodes during the while iterations.
 ...
 as the graph nodes.
 Both two processes generate unnecessary and duplicate nodes during the loop iterations.