...
\begin {table}[H]
% \vspace{-0.4cm}
    \caption{Results of the alternative {\THESYSTEM} implementation}
%     \vspace{-0.5cm}
        \label{tb:adapt-imp-alternatives}
        \begin{center}
        \centering
{\scriptsize
        \begin{tabular}{ | >{\tiny}r | c | c | c | c | c  |}
        \hline \hline
        \multirow{2}{*}{Program $c$}
         & \multicolumn{2}{c|}{$\THESYSTEM$-I}
         & \multirow{2}{*}{running time (second)} \\ 
         \cline{2-3}
         & {$\progA$ } & {$\query$\# } & \\ 
        %  \cline{2-6}
        %  ~  & $\THESYSTEM$-I & $\THESYSTEM$-II & $\THESYSTEM$-III  \\
         ...
         {$ \kw{loopMP(k)}$}  &  $ 2 + \max(1,2k) $ & $1 + 3k  $  &  ~ 0.0021 ~ \\
         {$ \kw{loop2MP(k)}$}  &  $ 2 + k $ & $1 + 3k  $  &  ~ 0.0021 ~ \\
         {$\kw{loop2RV(k)}$}&  $2 + 3k + k^2$ &  $1 + k + k^2$  &  ~ 0.0199 ~ \\
         $  \kw{mRCom(k, N)}$ & $ k   $ & $k $   &  ~ 0.0033 ~ \\
         $  \kw{mRCop(k)}$  & $  2k $ & $ 2k $   &  ~ 0.0052 ~ \\
