...
and the reachability-bound $\absclr(\absevent, c)$ for every edge $\absevent \in \absE(c)$ on the abstract transition graph,
...
x^l \in \progV(c) \land 
\hat{w} = 
\sum\left\{ \absclr(\absevent, c) \middle\vert \absevent \in \absE(c) \land \absevent = (l, \_, \_) \right\}
\right\}
\\
\progF & := & 
\left\{(x^l, n) 
