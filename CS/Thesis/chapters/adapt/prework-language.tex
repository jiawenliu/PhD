Previous works from {Thesis~\cite{weihao22}} formalized the adaptive data analysis into a while-like language named loop language with  limited expressiveness.
...
Figure~\ref{fig:evaluation} is a selection of rules of the trace-based operational semantics from {Thesis~\cite{weihao22}}.
...
This language model is limited in the following aspects:
 
\emph{Expressiveness.}
The loop command in this language model can only iterate constant number of times,
specifically the guard in the loop command can only be a constant number or an arithmetic expression which must be evaluated to a nature number.
Below we analyze the reasons.
%  By the operational semantics rule design, trace design, and their annotated query design, the loop iteration number can only be a constant number or an arithmetic expression evaluated to a nature number.
 ...
 Because 
%  there is information loss in their trace generated through the operational semantic rules.
%  The 
