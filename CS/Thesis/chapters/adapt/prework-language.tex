Previous works from {the thesis by~\cite{weihao22}} formalized the adaptive data analysis into a while-like language named loop language with limited expressiveness.
...
The expressions can be arithmetic, boolean expressions, or query expression.
The arithmetic and boolean expressions are standard.
Query expressions support the query requests in the data analysis.
The special variable $\chi$ represents a row of the database,
and access to values at a certain column in $\chi$, as $\chi[\aexpr]$.
However, the while loop iteration times cannot depend on the program state.
%
...
Each command is labeled with
% with a label $l$,
a natural number $l$ standing for the line of code where the command appears.
For the if statement, the label is associated with the conditional predicate $\bexpr$
and for the loop statement, it is associated with the loop counter $\aexpr$.
...
\paragraph*{Loop Map}
The loop map is a map from the label $l$ to the iteration number $n$.
...
\]
 ...
 To analyze the dependency relations between queries, the previous work
 designs a data structure, the \emph{annotated query}. 
% This is a key extension for analyzing the \emph{adaptivity}
Through this annotation, queries can be uniquely annotated 
% as $\mathcal{AQ}$,
 % and the annotation 
 by $(l,w)$, which considers the location of the query
 through the line number $l$ and
 the iteration index where this query is located
 if the query appears in a loop statement, specified by $w$.
 %% trace
\paragraph{Trace} 
...
Figure~\ref{fig:evaluation} is a selection of rules of the trace-based operational semantics from {the thesis by~\cite{weihao22}}.
We present only the rules related to query requests and the while loop evaluations.
% are selected here.
...
The sequence rule keeps tracking the modification of the trace, and the evaluation rule for the if command goes into one branch based on the result of the conditional predicate $\bexpr$. 
The rule \textbf{l-loop-a} first evaluates the loop counter $\aexpr$, when a loop counter is a constant, then the evaluation will start to execute the loop body.
...
This language model is limited in its expressiveness.
% the following aspects:
 
% \emph{Expressiveness.}
The loop command in this language model can only iterate a constant of times,
specifically, the guard in the loop command can only be a constant or an arithmetic expression that must be evaluated to a natural number.
Below we analyze the reasons.
In the rule \rname{l-loop}, a natural number $v_N$ is required on the premise of tracking the iteration times.
This requires that the loop iteration number has to be a natural number or evaluated to a natural number in advance to execute the loop.
Then in their trace and annotated query design,
the trace tracks the annotated query, which requires an integer annotation explicitly indicating the iteration number of the loop. For example, in the following program with a simple while loop,
 \[
 {\assign{x}{20}};
 \assign{y}{100};
 \ewhile (x < y) \edo 
 ...
 \}\}
 \] 
 the number of iterations cannot be evaluated to a natural number in advance of entering this loop. 
...
% Because 
% the trace only tracks the query requests, which lost the data dependency
% passed through
% the variables
% which are assigned by query result but not a query request.
