...
For the if statement, the label is associated to the conditional predicate $\bexpr$
and for the loop statement, it is associated to the loop counter $\aexpr$.
...
%   % For example, the loop map $w=[3:1, 4:2]$ indicates that the statement is currently in a nested loop, the outer loop starting from label $3$ and in its first iteration, the statement is now in the inner loop starting from label $4$ and in the second iteration. We use $\emptyset$ to represent an empty map, indicating the statement is not in any loop.
 ...
 To analyze the relation between queries, the previous work
...
% \emph{Expressiveness.}
The loop command in this language model can only iterate constant of times,
specifically the guard in the loop command can only be a constant or an arithmetic expression which must be evaluated to a natural number.
