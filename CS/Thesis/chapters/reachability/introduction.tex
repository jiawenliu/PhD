...
More in details, our algorithm first uses the difference constraints program abstraction model~\cite{SinnZV17,SinnZV14} enriched with boolean expressions to generate an abstract transition graph.
 %in Section~\ref{sec:psRB-progabs}
...
%in Section~\ref{sec:psRB-refine} and  Section~\ref{sec:psRB-rank}.
...
% and estimating its bound invariant in Section~\ref{sec:psRB-rank}.
...
%Section~\ref{sec:psRB-looprb} and~\ref{sec:psRB-pathrb} 
compute the \emph{reachability-bound} for each program point in a path-sensitive way.
%in Section~\ref{sec:psRB-psrb}.
We have implemented our algorithm in a prototype and evaluated it on 290 program extracted from different benchmarks. Our evaluation %Section~\ref{sec:psRB-eval} 
