\begin{example}
  ...
  It has two level nested loops. The outer level loop has two paths and the nested loop is in one path.
  ...
  \draw[ thick, -latex] (-8, 3.5)  -- node [left] {$j > 0$}  (-8, 0.5);
  \draw[ thick, -latex] (-7, 0.5)  to  [out=0,in=0] node [right] {$j' \leq j - 1$}  (-7, 3.5);
  \draw[ thick, -latex] (0.5, 10)  -- node [above] {$i \leq 0$}  (5.5, 10);
  ...
  \textbf{Refined Program}:
    ...
    $\lpchB(1: \rprog_1^1, \tpath_1) = \frac{k}{4}$ \quad
    $\lpchB(1: \rprog_1^1, \tpath_3) = \frac{k}{4}$ \quad 
    $\lpchB(1: \rprog_1^1, \tpath_5) = \frac{k}{4}$ \\ 
    {$\lpchB(1: \rprog_1^1, \tpath_2) = \frac{k}{4}$} \quad
    $\lpchB(1: \rprog_1^2, \tpath_1) = \frac{k}{4}$ \quad
    $\lpchB(1: \rprog_1^2, \tpath_3) = \frac{k}{4}$ \\ 
    $\lpchB(1: \rprog_1^2, \tpath_5) = \frac{k}{4}$ \quad 
    \highlight{$\lpchB(1: \rprog_1^2, \tpath_2) = \frac{k}{4}$} \\
    $\lpchB(5: \rprepeat(\tpath_2), \tpath_2) = n$ 
    %
    %
    ...
    $\psRB(2) = \frac{k}{2} $ \\
    $\psRB(3) = \psRB(4) = \psRB(7) = \frac{k}{4} $ \\
    \highlight{$\psRB(6) = \frac{k}{4} \times k$} \\
