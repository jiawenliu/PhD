...
This bound is referred to as the \emph{reachability-bound} in the program analysis area,
...
It over-approximates the \emph{reachability-bound}s on different paths inside a while loop.
% \\
...
by inferring the bounds on the loop iteration numbers,
 ...
 \\
 None of them computes the \emph{reachability-bound} on a given program control location directly or path-sensitively.
 To leverage these limitations,
we introduce a path-sensitive \emph{reachability-bound} analysis algorithm in this paper, which aims to solve 
the \emph{reachability-bound}s problem efficiently and path-sensitively.
