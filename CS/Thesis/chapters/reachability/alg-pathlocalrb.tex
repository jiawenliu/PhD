...
\begin{enumerate}
  ...
  based on the estimated ranking function from Section~\ref{sec:psRB-rank} and estimate the
...
each $\tpath$ w.r.t. its \textbf{the innermost loop}, $\kw{enclosed}(\rprog, \tpath)$ where $\tpath$ is located.
