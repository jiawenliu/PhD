...
Our approach uses the ranking function invariant to compute 
% a precise progress invariant inspired from~\cite{GulwaniJK09}.
% We first compute 
three abstract states for each ranking function in different subprograms of a refined program as below.
...
\begin{lem}[Soundness of the Path Local Reachability-bound]
  ...
  if the loop program $l: \rprog_l = \kw{enclosed}(\rprog, \tpath)$ is the closest loop where $\tpath$ is nested in this program,
