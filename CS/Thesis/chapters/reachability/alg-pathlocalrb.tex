...
\begin{enumerate}
  ...
  \item Estimated by Equation~\ref{eq:absBD} in Section~\ref{sec:psRB-loopbound-rankbased} path-insensitively based on the estimated ranking function from Section~\ref{sec:psRB-rank}. This bound is precise when there is only one path in this loop but not for loops with multiple paths.
