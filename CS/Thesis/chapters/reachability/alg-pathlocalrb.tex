...
the \emph{Variable Gradient Decent}-- 
 $\varGD(\rprog', c)$-- an invariant of the minimum amount decreased in one execution of $\rprog_l$ for the ranking functions in $\rprog$, as follows, 
...
$\varGD(\rprog, c)$ for the ranks in a program $\rprog$ is computed as follows.
