\begin{table}[ht]
    \caption{The Complete Effectiveness Evaluation Table of {\PSRB}}
    \label{tb:effectiveness-eval-appendix}
    \centering
    {\scriptsize
    \begin{tabular}{ | >{\scriptsize}c | >{\scriptsize}c | >{\scriptsize}c | >{\scriptsize}c | >{\scriptsize}c | c | c | c | c | c | c | c | c | c | c | c | c | c |}
    \hline
    {Bench-} & {theory} & ($\# p$) & ($\# l$) & \multicolumn{6}{c|}{reachability-bound ($\pointnum$)} & \multicolumn{5}{c|}{worst bound ($\# p$)} \\
    \cline{5-15}
   ...
   \begin{table}[ht]
    \caption{The Complete Accuracy Evaluation Table of {\PSRB}}
    ...
    \hline
    {Bench-} & {theory} & ($\# p$) & ($\# l$) & \multicolumn{2}{c|}{overall complexity bound}\\
    \cline{5-6}
