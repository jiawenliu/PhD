...
% \item $l_0$ is either the program point of a looping header or the program entrance ($l_0 = 0$),
% i.e., $l_0 \in \loopl(c) \cup \{ 0 \}$
% \item and $l_n$ is either the program point of a looping header or the program exit ($l_n = \lex$),
...
Look again at the walk-through an example in Figure~\ref{fig:relatedNestedWhileOdd-overview}(b).
...
\begin{example}[Two Counter While]
 ...
 So over its abstract transition graph generated by Section~\ref{sec:psRB-progabs} as Figure~\ref{fig:whileTwoCounters-overview}(b), we collect all its simple transition path as in Figure~\ref{fig:whileTwoCounters-overview}(c).
