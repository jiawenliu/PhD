...
\begin{defn}[Simple Transition Path]
 \label{def:tpath}
Given an abstract transition graph $\absG(c)$ of a program $c$,
a \emph{simple transition path} of this program
is a
simple path on $\absG(c)$ such that 
% either a simple cyclic path, which has the same 
its start- and end-point
% or a simple path has either different while 
has to be either the
loop headers, or the program entrance, or exit,
%  as its start- and end-point
and it cannot visit any loop header except the start- and end-point.

In other words, a path $l_0 \xrightarrow{dc_0} l_1 \xrightarrow{dc_1} \ldots l_n \in \paths(\absG(c))$ with the
vertex sequence $(l_0, \ldots, l_n)$, where $l_i \in \absV(c)$ for every $i = 0, \ldots, n$ and
%
...
For example in Figure~\ref{fig:relatedNestedWhileOdd-overview}(b), path $1 \to 2 \to 8 \to 1$ is a \emph{simple transition path}.
However, $1 \to 2 \to 3 \to 4 \to 5 \to 4 \to 6 \to 7 \to 1$ is not a \emph{simple transition path} because it is not simple.
Also, path $1 \to 2 \to 3 \to 4 \to 6 \to 7 \to 1$ is not a \emph{simple transition path} because it visits a loop header $4$ inside the path. 
The path $1 \to 2 \to 3$ has end-point $3$, which isn't a loop header. In this sense, it is not a \emph{simple transition path} either.
In Figure~\ref{fig:threeWhile-overview}(b), path $1 \to 2 \to 3 \to 4 \to 5 \to 6$ is not a \emph{simple transition path} on the graph $\absG(\kw{threeNestedWhile}(n, m, N))$ because it visits a loop header $3$ inside the path.
...
This algorithm first compute the execution orders of
