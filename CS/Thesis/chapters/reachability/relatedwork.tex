...
In analyzing the loop bounds, the unified lattice model by~\cite{CousotH78}, polyhedra by~\cite{CousotC77} and octagons by~\cite{Mine06} are used in~\cite{GulwaniZ10} for summarize the loop transition and generating the transition system.
...
The wedge abstract developed by~\cite{KincaidCBR18} for generating the non-linear loop invariant
...
We also implement part of the algorithm of Loopus based on the transition bound algorithm by~\cite{SinnZV17} in terms of program abstraction and variable invariant generation,
...
The works in analyzing the program complexity including~\cite{GustafssonEL05, HumenbergerJK18} only focus on estimating 
...
For the same reason, the tools CofloCo from works in~\cite{Montoya17, Flores-MontoyaH14, Flores-Montoya16}, KoAT by~\cite{BrockschmidtEFFG16, BrockschmidtEFFG14, FalkeKS12, FalkeKS11}, and the amortized algorithm in~\cite{LuCT21} which we evaluated over our benchmarks all give only the entire loop bound path-insensitively.
...
\cite{JostHLH10} use linear programming to infer bounds for functional programs, but they are restricted to computing only linear bounds.
Other techniques that are based on
type system (e.g.~\cite{CicekBG0H17, RajaniG0021}), the cost analysis via effect systems (e.g.~\cite{CicekBG0H17, RadicekBG0Z18, QuG019}), and Hoare logic (e.g.~\cite{CarbonneauxHS15})
...
For example, the termination analysis through ranking functions originated from paper~\cite{BradleyMS05} and followed up papers such as~\cite{FalkeKS11, FalkeKS12, CookSZ13, Zuleger18} are widely developed into bound analysis including~\cite{AliasDFG10, SinnZV14, BrockschmidtEFFG16, BrockschmidtEFFG14}.
Loop summarization and invariant generate through path refinement~\cite{ManoliosV06, BalakrishnanSIG09, SharmaDDA11, Flores-MontoyaH14, HumenbergerJK18, CyphertBKR19} techniques are also extended into precise resource cost and loop bound estimation based on multiple loop path refinement e.g.~\cite{GulwaniJK09, ZulegerGSV11}. 
% Invariant Generation Through Ranking functions~\cite{AliasDFG10} for multipath loops.
\cite{KincaidBCR19} introduce a precise closed-form loop summarization techniques which can help to improve the accuracy of the path refinement, as well as the non-linear loop summarization techniques in~\cite{KincaidCBR18} and the invariant generating algorithm considering recurrence in~\cite{BreckCKR20}. They are all heavy compare to our light-weight path refinement implementation adapted from~\cite{GulwaniJK09}.
