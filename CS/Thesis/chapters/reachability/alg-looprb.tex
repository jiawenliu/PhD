...
the loop initial state $\lpinit(\rprog, \tpath, c)$, and the loop next state $\lpnext(\rprog, \tpath, c)$ based on the refined program below.
...
two loop abstract states
% the \emph{loop initial state} $\lpinit(\rprog, \tpath, c)$, and the \emph{loop next state} $\lpnext(\rprog, \tpath, c)$
w.r.t. the ranking functions as follows.
 
1. The \emph{loop initial state} 
