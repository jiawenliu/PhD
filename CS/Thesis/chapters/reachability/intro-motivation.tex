...
\footnotetext{We use the notation $(l_0 \to \ldots \to l_n)$ to denote a vertices sequence $(l_0, \ldots, l_n)$, and the constraint on each edge in
...
Among works on program complexity, cost and loop bound analysis, \cite{GulwaniJK09} computes the tight bound on the loop iteration but not precise \emph{reachability-bound}s on different location.
...
To compute the bounds for locations on different paths of a loop, we compute a new quantity, the \emph{path reachability-bound},
% which is the first key idea of this path-sensitive \emph{reachability-bound} analysis algorithm. 
This bound approximate the evaluation times of each \emph{simple transition path} during the program execution, where \emph{simple transition path} is loop free and interleaving free.

This bound is computed based on the refined program where the interleaving of different \emph{simple transition path}s are explicit.
%  and using the estimated ranking function for every path, combines two lines of work introduced in Section~\ref{sec:reachability-background}. It is benefited from the high accuracy of the path refinement and the ranking function estimation, but reduces the efficiency comparing to simply computing the ranking function.
...
Then it refines every loop in $\kw{twoPathsWhile}$ by explicitly computing the interleaving of different paths and
...
\]
Each $\tpath_i$ in this refined program is a \emph{simple transition path} we computed in a pre-procedure, which is loop free and not interleaving with the other $\tpath_j, j \neq i$ as in Figure~\ref{fig:whileTwoCounters-overview}(c).
...
In this sense, we introduce and compute the \emph{loop reachability-bound}, $\lpchB(L:\rprog, \tpath)$.
...
we compute \emph{loop reachability-bound} for each $\tpath_i$ w.r.t. each of its outer loop.
