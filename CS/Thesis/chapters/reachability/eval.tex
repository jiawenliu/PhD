...
complexity analysis we aim to answer the following two questions: is our tool able to compute precise reachability-bound for different program points (\emph{Effectiveness})? If so, can we also have better overall complexity estimation (\emph{Accuracy})?
...
\begin{table}[ht]
    ...
    \centering
    {\scriptsize
    \begin{tabular}{ | >{\tiny}c | >{\scriptsize}c | >{\scriptsize}c | >{\scriptsize}c | >{\scriptsize}c | c | c | c | c | c | c | c | c | c | c | c | c | c |}
    \hline
    \hline
