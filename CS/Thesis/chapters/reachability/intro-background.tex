...
Then we present the algorithm for estimating the two quantities in Section~\ref{sec:psRB-looprb} and~\ref{sec:psRB-pathrb} and compute the \emph{reachability-bound} for each program point path-sensitively in Section~\ref{sec:psRB-psrb}.
Our evaluation on the prototype implementation in Section~\ref{sec:psRB-eval} shows that we can accurately estimate different bounds for different program points on multiple loop paths. The main contributions are as follows.
