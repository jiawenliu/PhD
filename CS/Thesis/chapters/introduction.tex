...
\begin{enumerate}
   ...
   analyzed. This is presented in Section~\ref{subsec:dynamic-datadep}.
  %   and 
  ...
  % property of programs. This analysis is developed 
  % \\
   \item The second step in Section~\ref{subsec:dynamic-reachability} is to analyze the \emph{dependency quantity} 
  %  analysis, 
  ...
  % \\
   \item The last step in Section~\ref{subsec:dynamic-adapt}  is the intuitive \emph{adaptivity} quantity analysis, 
...
I will develop a program analysis algorithm, named {\THESYSTEM} in Chapter~\ref{ch:static}.
%  which 
...
% and accurately.
\item The data dependency relation analysis through the static data flow analysis technique in Section~\ref{subsec:alg_weightedgegen}.
\item Still in Section~\ref{subsec:alg_weightedgegen}, the dependency quantity analysis through the static program reachability bound analysis techniques.
\item 
% The program  estimation, 
In Section~\ref{subsec:static-adapt},
...
improve in my full-spectrum analysis as follows in Section~\ref{ch:improved} in following 3 aspects.
% \begin{enumerate}
...
% \end{enumerate}
\begin{enumerate}
   \item The {\tt Query While} Language is extended in Section~\ref{subsec:refine-exe-language} with inter-procedure call.
   \item In the execution-based \emph{adaptivity},
   the precision of formalizing the intuitive \emph{adaptivity} is improved,
%  in the formal  model 
in the meantime extend this analysis with inter-procedure call in Section~\ref{sec:refine-exe}.
\item In static \emph{adaptivity} analysis, I 
give a tighter estimated upper bound on \emph{adaptivity}
%  give a tighter estimated upper bound 
Specifically, I improve the accuracy of the static \emph{dependency quantity} analysis in the second step through 
path sensitive reachability bound analysis techniques in Section~\ref{subsec:refine-static}.
% \item In the third step of static program analysis, I will improve the accuracy of the adaptivity computation algorithm,
% compute a tighter adaptivity upper bound as well.
\end{enumerate}
% \\
...
% I'm interested 
These two observations motivated my in 
...
\section{Furture Works}
\label{sec:intro-cfl}
Still in the area of general program resource cost analysis,
the traditional methodologies of performing data flow and control analysis and 
