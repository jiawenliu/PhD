\begin{lemma}[Soundness of The Abstract Execution Trace]
  ...
  \begin{proof}
    ...
    \\
    In case of $l' = l$, by the abstract execution trace computation in Definition~\ref{def:psRB-absevent_compute}, we have 
    \[
    ...
    \\
    In case of $l = l_w$, let $\event = (b, l_w, \etrue)$ and $\trace' =  [(b, l_w, \etrue)]$, then it is sufficient to show.
      \[
        \exists 
        \absevent \in \absflow(c) \land \absevent = (l, \_, \_)
      \]
      By abstract trace computation in Definition~\ref{def:psRB-absevent_compute}, we have
      \[
    ...
    \[
      \absflow(c) = \absflow'(c; \clabel{\eskip}^{\lex}) = \{(l_w, \top, \init(c_w))\} \cup \cdots.
    \]
    Let $\absevent = (l_w, \top, \init(c_w))$, then we have this case proved.
