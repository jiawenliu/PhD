...
\begin{proof}
  ...
  \[
    \config{c, \trace_0} \rightarrow \config{c', \trace}.
  \]
  By induction on $c$, we have the same cases as the Proof of Lemma~\ref{lem:tracenondec} in Appendix~\ref{apdx:tracenondec} excepting the case \textbf{$c = \clabel{\assign{x}{\query(\qexpr)}}^{l'}$}.
  ...
  \\
  Formally, the two cases will be proved in the same way as $\ewhile \clabel{b}^{l} \edo (c_w)$.
\end{proof}
    %
