...
\\
Since $\tpath' \neq \tpath$, we know $\tpath$ isn't executed when executing $c_l$, i.e., 
$$\lcounter(\trace, \pathl(\tpath)) = 0.$$
Then we have
\[
...
In this case, by Definition~\ref{alg:alg-refine_rewrite}, we have 
...
\]
Let $c' \in \cdom$ be the while language program corresponding to $\rprog'$ such that 
$$\algrewrite(c') = \rprog'.$$
...
\\
By Lemma~\ref{lem:loopbound_sound}, we know $BD(\rprepeat(\rprog'), c)$ is an upper bound on the number of iterations of $\rprepeat(\rprog')$. Then we have
\[
...
\[
  ...
  \]
  Let $c_l \in \cdom$ be the while language program corresponding to $\rprog_l$ such that 
  $$\algrewrite(c_l) = \rprog_l.$$
...
\[
  \econfig{ BD(\rprepeat(\rprog'), c) \times \outinB(\rprog', \tpath)} (\trace_0) \geq \lcounter(\trace, \pathl(\tpath)) 
\]
Given the execution of the loop $\rprepeat(\rprog')$ is non-terminating,
by Lemma~\ref{lem:loopbound_sound} second subcase in the non-terminating case, we know
\[
