...
By the definition $\eventdep(\event_1, \event_2, [\event_1; \event_2], c, D)$,  we know
\begin{equation}
...
\end{equation}
By 
% Inversion Lemma~\ref{lem:inv_skip} and 
the execution in Eq.~\ref{eq:valdep_inv1}, we know:
 \[
 c_1 =_c 
 [\assign{{x}_2}{\expr_2 / \query(\qexpr_2)}]^{l_2};{c}_2
...
we know there is a $\ewhile$ conditional command
 $(\ewhile [b_w]^{l_w} \edo (c_w))$ in $c_2$ and
...
\[
  \forall z^r \in \lvar(c) \setminus \{x_1^{l_1}\} ,
