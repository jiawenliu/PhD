...
\\
Let $m_i$ be the evaluation results of $\pathsearch_{scc}(\sccgraph_i)$ for every $k_i$ as follows,
\[
  \config{\pathsearch(c), \trace_0} \earrow m_i
\]
...
\begin{lem}[Soundness of $\pathsearch_{scc}$]
  ...
  For every program $c$, given its \emph{estimated dependency graph} $\progG$, if $\sccgraph$ is a strong connected sub-graph of $\progG$, then
  $\max\{\qlen(k) | k \in \walks(\sccgraph)\} \leq \pathsearch_{scc}(\sccgraph) $.
  %
  \[
    \forall c \in \cdom, \sccgraph \sthat  \sccgraph \subseteq_{\kw{graph}} \progG(c)
    \implies 
    \max\{\qlen(k) | k \in \walks(\sccgraph)\} \leq \pathsearch_{scc}(\sccgraph) 
    \]
\end{lem}
...
%
  ...
  $(x, s_1, \ldots, y)$, it is sufficient to show:
  \[
    \qlen(k_{x,y}) = \len(s | s \in (x, s_1, \ldots, y) \land \qflag(s) = 1) \leq \pathsearch_{scc}(\sccgraph)
  \]
  ...
  $$
  \len(s | s \in (x, s_1, \ldots, y) \land \qflag(s) = 1) \leq \kw{dfs(\sccgraph, x, visited)}.
  $$
  %
 ...
 there  is a path $p_{x,y} $ from $x$ to $y$ with this vertices sequence: $(x, v_1, \ldots, y)$.
  ...
  % \\
  $$ 
  \len(s | s \in (x, s_1, \ldots, y) \land \qflag(s) = 1) \leq \kw{flowcapacity}(p_{x, y}) \times \kw{querynum}(p_{x, y} ) 
  $$
  %
...
$\visit(s) (x, s_1, \ldots, y)  \leq \kw{flowcapacity}(p_{x, y})$
  ...
  \\
  % Then we have a potential walk $k_{x,x}$ with vertices sequence $(x, s_1, \ldots, y) + (v_1', \ldots, x)$, and
  % we know $\qlen(k_{x,y}) \leq \qlen(k_{x,x})$.
  % Then, it is sufficient to show 
  % $$
  % \qlen(k_{x,x}) = \len(s | s \in (x, s_1, \ldots, y, v_1', \ldots, x) \land \qflag(s) = 1) \leq \kw{dfs(\sccgraph, x, visited)}
  % $$
  %
  % By line:13 of the $\kw{dfs(\sccgraph, x, visited)}$ in Algorithm~\ref{alg:adaptscc},
  % \\
  ...
  % \\
  $$ 
  \len(s | s \in (x, s_1, \ldots, y) \land \qflag(s) = 1) \leq \kw{flowcapacity}(p_{x, x}) \times \kw{querynum}(p_{x, x}) 
  $$
  %
  % \\
...
$\visit(s) (x, s_1, \ldots, y)  \leq \kw{flowcapacity}(p_{x, y})$
  ...
  Then we know 
  \\
  $\len(s | s \in (x, s_1, \ldots, y) \land \qflag(s) = 1) \leq \kw{flowcapacity}(p_{x, y}) \times \kw{querynum}(p_{x, y}) = r[y]$
  \\
  ...
  $\len(s | s \in (x, s_1, \ldots, y) \land \qflag(s) = 1) \leq r[x]$
  \\
  This case is proved.
  %
%
