...
\begin{lem}[Control Dependency Inversion]
	...
	For every $c \in \cdom$, $D \in \dbdom, \trace \in \ftdom$ and two assignment events $\event_1, \event_2 \in \eventset^{\asn} $, if they are in the second case of the \emph{Event May-Dependency} relation from Definition.~\ref{def:event_dep},
	...
	\begin{equation}
		...
		\\ \quad \implies	  
			...
			\tlabel_{\trace_3} \cap \tlabel_{\trace_3'} = \emptyset
			  ...
			  \land \event_2 \not\eventin \trace_3'
		\\ \quad \implies	
		\forall z \in VAR(\pi_1(\event_b)) \sthat  
...
$\event_1, \event_2 \in \eventset^{\asn}$,
if the trace $trace$ has the form $\trace = [\event_1] \tracecat \trace' \tracecat [\event_2]$ with $\trace' \in \ftdom$, and $\eventdep(\event_1, \event_2, \trace, c, D)$
then $\flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c) $,
...
$
and $\flowsto(\pi_1(\event)^{\pi_2(\event)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c)$.
%
	\[
	\begin{array}{l}
	...
	\]
\end{lem}
The lemmas used in this proof are Lemma~\ref{lem:inv_indepevents}, Lemma~\ref{lem:ctldep_inv}, and the Inversion Lemmas~\ref{lem:inv_expr}, \ref{lem:inv_expr_gnl},
\ref{lem:inv_event}, and \ref{lem:inv_live}.
This Lemma is proved by showing the existence of contradiction.
\begin{proof}
	Taking arbitrary 
...
\begin{lem}[Independent Events Doesn't Block $\flowsto$ ]
	...
	\begin{itemize}
		...
		\begin{array}{l}
			...
			\\ \quad 
			\implies 
			\left( \forall \event \in \trace' \sthat  \neg \eventdep(\event_1, \event, \trace[\event_1:\event], c, D)
			\lor \neg \flowsto(\pi_1(\event)^{\pi_2(\event)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c) 
			\right) 
			\\ \quad 
			\implies 
...
\begin{array}{l}
	...
	\\ \quad 
	\implies 
	\Big( \forall \event \in \trace' \sthat  
	\neg \eventdep(\event_1, \event, \trace[\event_1:\event], c, D)
	\lor  \pi_1(\event) \notin VAR(\pi_1(\event_2))
	\Big) 
	\\ \quad 
...
taking arbitrary trace has the form $\trace = [\event_1; \ldots; \event_2]$ for arbitrary $\trace_2 \in \ftdom$,
 then we know $\exists \trace' \in \ftdom \sthat  \trace = [\event_1] \tracecat \trace' \tracecat [\event_2]$, let $\trace_2$ be this $\trace'$.
%
...
\trace_2' \in \ftdom, c_1, c_2 \in \cdom$ as the events, traces and commands satisfying the definition,
...
\[
	\begin{array}{l}
		\forall x^i \in \lvar, \trace, \trace' \in \ftdom, v, v' \sthat 
		\\ \qquad 
		\Big( \forall z^j \in \lvar / \{\pi_1(\event_z)^{\pi_2(\event_z)} \} \sthat  
		\\ \qquad 
...
\]
	\[
		\begin{array}{l}
			\forall x^i \in \lvar, \trace, \trace' \in \ftdom, \qval, \qval' \sthat 
			\\ \qquad 
		\Big( \forall z^j \in \lvar / \{\pi_1(\event_z)^{\pi_2(\event_z)} \} \sthat  
		\\ \qquad 
		\env(\trace) z = \env(\trace') z \Big) \land 
...
% \[
% 	\forall \event_z \in \kw{use}_{\trace_2} \sthat 
...
% \[
% 	\begin{array}{l}
% 		\forall \event_z \in \eventset^{\asn} \sthat  
% 	\event_z \in \diff_{\eventset} \Leftrightarrow 
% 	\exists \trace_2^h, \trace'^h_2, \trace_2^t, \trace'^t_2, \event_z' \in \trace_2' \sthat  
...
% \\
% and $\forall z^j \in (\lvar \setminus (\mathbb{LV}_{\diff_{\eventset}} \cup\{\pi_1(\event_1)^{\pi_2(\event_2)}\}) ) \sthat  
% \env(\trace_1 \tracecat [\event_1] \tracecat \trace_2) z = \env(\trace_1 \tracecat [\event_1'] \tracecat \trace_2') z $,
% \\
...
% $\forall \event_z', \event_2'' \in \eventset^{\asn}, \trace_z',\trace_z,  \trace_1' \in \ftdom, c_z \in \cdom$ satisfying following two executions, we have $\event_2 \eventeq \event_2''$.
% \[
% 	\begin{array}{l}
% 		\config{c, \trace_0} \rightarrow^{*}
...
% \[
% 	\forall z^j \in (\lvar \setminus \mathbb{LV}_{\diff_{\eventset}} ), \trace, \trace' \in \ftdom, v, v' \sthat  
% 	\env(\trace) z = \env(\trace') z 
% 	\land 
% 	\config{\trace, \expr_2} \aarrow v 
% 	\land 
...
% \\
% $\forall z^j \in (\lvar \setminus \diff_{\eventset} \cup\{\event_1\} ) \sthat  
% \env(\trace_1 \tracecat [\event_1] \tracecat \trace_2) z = \env(\trace_1 \tracecat [\event_1'] \tracecat \trace_2') z $.
% \\
% By $\forall \event \in (\trace[\event_1:\event_y]\setminus \{\event_1, \event_y\}) \sthat 
...
% 	%
% 	For every $D \in \dbdom , c \in \cdom, \trace \in \ftdom$, an assignment event
% 	$\event_1 \in \eventset^{\asn}$ and a test event $\event_2 \in \eventset^{\test}$,
% 	if the trace $trace$ has the form $\trace = [\event_1] \tracecat \trace' \tracecat [\event_2]$ with $\trace' \in \ftdom$, 
% 	and $\eventdep(\event_1, \event_2, \trace, c, D)$,
...
% \label{lem:inv_alg1}
% For all $D \in \dbdom , c \in \cdom, \trace \in \ftdom, \event_1, \event_2 \in \eventset^{\asn}$, and a list $l$,
...
% \[
% \begin{array}{l}
%     \forall D \in \dbdom , c \in \cdom, \trace \in \ftdom, l \sthat  \forall \event_1, \event_2 \in \eventset^{\asn} \sthat 
%   \\ \quad 
...
% % \]
% % for $S=\kw{setfilter}
% % 		(\efun l \to \exists l' \sthat  l = \event_1 :: l' ++ [\event_2]) ~ \kw{dfs}(\trace, c, D)$.
...
% $$
% %
% where $S=(\kw{setfilter} ~(\efun l \to l = [\event_1, \ldots, \event_2]) ~ (\kw{dfs} \eapp \trace \eapp c \eapp  D))$.
% \\
...
% $$
%  %
% Since $l_e \in (\kw{setfilter} ~(\efun l \to l = [\event_1, \ldots, \event_2]) ~ (\kw{dfs} \eapp \trace \eapp c \eapp  D))$,
% \\
% by the $\kw{setfilter}$ function,
% we know only the lists of events in $(\kw{dfs} \eapp \trace \eapp c \eapp  D)$ having the form
% $ [\event_1, \ldots, \event_2] $ are preserved in $S$, i.e.,
% \[
% 	\forall l \in (\kw{setfilter} ~(\efun l \to l = [\event_1, \ldots, \event_2]) ~ \kw{dfs}(\trace, c, D))
% 	\sthat  l = [\event_1, \ldots, \event_2]
% \]
% %
% Then we know $l_e$ also has the same form, 
% i.e., $l_e = [\event_1, \ldots, \event_2]$.
% %
% \\
...
% \[
% 	[\pi_1(\event_1)^{\pi_2(\event_1)}, \ldots, \pi_1(\event_2)^{\pi_2(\event_2)}]
% \]
% %
% %
% This lemma is proved.
...
% 	\\ \qquad \qquad
% 	(\kw{setfilter} ~
% 		(\efun l \to l = [\event_1, \ldots, \event_2]) ~ 
% 		% \left(\left\{[\event_2]\right\} \cup \left\{ \event_1 \sthat ackrel{[\event_1; \event_2]}{\uplus} [\event_2] \right\} \right))
...
% \[
% 	\begin{array}{l}
% 	% \event_1 \sthat ackrel{[\event_1; \event_2]}{\uplus} [\event_2]
...
% 	\[
% 	\begin{array}{l}
% 		\forall x \in \lvar_c, \kw{V_{ptl}} \in \subseteq \lvar_c \expr \sthat  
% 		\exists \trace, \trace' \in \ftdom \sthat  
% 		\\ \quad
% 		\forall z \in \lvar_c \setminus (\{x\} \cup \kw{V_{ptl}}) \sthat  
% 		\env(\trace) z = \env(\trace') z 
% 		\\ \quad \land
% 		\forall \event \in \trace, \event' \in \trace' \sthat  
...
% %
% \[
%   \forall D \in \dbdom , c \in \cdom, \event_1, \event_2 \in \eventset^{\test}, \event_3 \in \eventset \sthat 
...
% 	\[
% 	\forall c \in \cdom, D \in \dbdom , \event_1, \event_2, \event_3 \in \eventset^{\asn}\sthat  
...
$\vardep(y^j, z^l, c)$, then $\vardep(x^i, z^l, c)$.
	\[
	\forall c \in \cdom, x^i, y^j, z^l \in \lvar_c \sthat  
	\vardep(x^i, y^j, c) 
	\land
	\vardep(y^j, z^l, c) 
	\implies
