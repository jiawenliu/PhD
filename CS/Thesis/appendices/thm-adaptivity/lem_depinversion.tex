...
then there is either a flows to relation,
$\flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c) $,
or 
exists
...
\[
	\begin{array}{l}
		\forall x^i \in \lvar(c), \trace, \trace' \in \ftdom, v, v' \sthat 
		\\ \qquad 
		\Big( \forall z^j \in \lvar(c) / \{\pi_1(\event_z)^{\pi_2(\event_z)} \} \sthat  
		\\ \qquad 
...
\]
	\[
		\begin{array}{l}
			\forall x^i \in \lvar(c), \trace, \trace' \in \ftdom, \qval, \qval' \sthat 
			\\ \qquad 
		\Big( \forall z^j \in \lvar(c) / \{\pi_1(\event_z)^{\pi_2(\event_z)} \} \sthat  
		\\ \qquad 
		\env(\trace) z = \env(\trace') z \Big) \land 
...
\]
 ...
 \\ 
$
	\forall z^l \in \lvar(c) \setminus ( \lvar_{\trace_2} \setminus \lvar_{\kw{use}_{\trace_2}}), 
