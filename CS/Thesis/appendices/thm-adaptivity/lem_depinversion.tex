...
then either there is a flows to relation,
$\flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c) $,
or 
there exists
...
%
	...
	\]
\end{lem}
The lemmas used in this proof are Lemma~\ref{lem:inv_indepevents_asn,lem:inv_indepevents_ctl}, Lemma~\ref{lem:ctldep_inv}, and the Inversion Lemmas~\ref{lem:inv_expr}, \ref{lem:inv_expr_gnl},
...
\begin{proof}
	...
	\\
	Let $ \event \in \eventset$ be a trace such that
	 \[
		\event \in \trace' \land
	\eventdep(\event_1, \event, \trace[\event_1:\event], c, D)
	\land 
	\flowsto(\pi_1(\event)^{\pi_2(\event)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c)
	\]
	By type of $\event$, we have two cases,
	\caseL{$\event \in \eventset^{\asn}$}
	This case is proved by Lemma~\ref{lem:inv_indepevents_asn}.
	\caseL{$\event \in \eventset^{\test}$}
	...
	% \end{array}
	% \]
	we prove the two cases in Lemma~\ref{lem:inv_indepevents_asn} and~\ref{lem:inv_indepevents_ctl}.
\end{proof}
%
%
%
...
\config{c_1, \trace_1 \tracecat [\event_1']} \rightarrow^{*} \config{c_2, \trace_1 \tracecat [\event_1'] \tracecat \trace_2' \tracecat [\event_2']} 
\\
\land \diff(\event_2,\event_2' ) ~\star \land 
...
\\
Taking arbitrary $\event_z \in \trace_2$ and $\event_z \in \eventset^{\asn}$, we know 
\[
\neg \eventdep(\event_1, \event, \trace[\event_1:\event_z], c, D)
\lor  \neg \flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c).
\]
...
\[
	(\pi_4(\event_2) = \bullet \land \pi_4(\event_2') = \bullet) \land \pi_3(\event_2) = \pi_3(\event_2')
	\]
	or
	\[
		(\pi_4(\event_2) \neq \bullet \land \pi_4(\event_2') \neq \bullet) \land \pi_4(\event_2) = \pi_4(\event_2')
		\]
Then by Definition~\ref{def:diff}, we know
\[
	\neg \diff(\event_2, \event_2').
	\]
This is contradicted to $\star$ in the assumption, and we have this case proved.
%
\subcaseL{$ \neg \flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c)$}
