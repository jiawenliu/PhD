...
% $
% and $\flowsto(\pi_1(\event)^{\pi_2(\event)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c)$ and 
% $\event \in \eventset^{\asn}$
% if $\eventdep(\event_1, \event_2, \trace, c, D)$ satisfies
% the Equation~\ref{eq:eventdep}(b)
% or 
% $
% \eventdep(\event_1, \event, \trace[\event_1:\event], c, D)
% $
% and $\flowsto(z^l, \pi_1(\event_2)^{\pi_2(\event_2)}, c)$ and $\event \in \eventset^{\test}$
% for every $z \in \kw{FV}(\pi_2(\event))$ and some label $l \in \lvar(c)$
% if $\eventdep(\event_1, \event_2, \trace, c, D)$ satisfies
% the Equation~\ref{eq:eventdep}(c).
% %
% 	\[
% 	\begin{array}{l}
...
% 		\big(\eventdep(\event_1, \event, \trace[\event_1:\event], c, D)
% 		\land 
% 		\event \in \eventset^{\asn} \big) 
% 		\\ \qquad \qquad
% 		\land \flowsto(\pi_1(\event)^{\pi_2(\event)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c)
% 		% \\ \qquad \quad
% 		% \land 
% 		% \big(\eventdep(\event_1, \event, \trace[\event_1:\event], c, D) \land \event \in \eventset^{\test} \big)
% 		% \\ \qquad \qquad
% 		% \implies \forall z \in \kw{FV}(\pi_2(\event)) \sthat \exists l \in \lvar(c) \sthat
...
\begin{lem}[Equation~\ref{eq:eventdep}(c) Inversion]
	...
	and $\event_b \in \eventset^{\test}$ be the witness of the testing event,
