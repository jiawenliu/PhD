\begin{lem}[Expression Inversion]
	\label{lem:inv_expr}
	For all {$ x^i \in \lvar$, and $\trace, \trace' \in \tdom$, and an expression $\expr$} if
...
\begin{lem}[Expression Inversion Generalization]
	...
	\begin{itemize}
		\item $\expr$ is an arithmetic expression $\aexpr$,
		% \\ 
		and for all $z^j \in \lvar \setminus \diff, \trace, \trace' \in \tdom, v, v'$ such that 
		...
		there exist $\trace, \trace' \in \tdom, v, v'$ such that 
		$\env(\trace) z = \env(\trace') z$, and 
		$
		...
		\[
			...
			\\ \quad
			\forall z^j \in \lvar \setminus \diff, \trace, \trace' \in \tdom, v, v' \sthat  
			\\ \quad
			...
			\exists \trace, \trace' \in \tdom, v, v'\sthat  
			\\ \quad
			\env(\trace) z = \env(\trace') z \land 
...
% 
	...
	for all $ z^j \in \lvar \setminus \diff, \trace, \trace' \in \tdom, \qval, \qval' \sthat  
 \env(\trace) z = \env(\trace') z \land 
 \config{\trace, \qexpr} \qarrow \qval \land \config{\trace', \qexpr} \qarrow \qval' \land \qval =_q \qval'$;
 and for all 
	$ z^j \in \lvar / (\diff \cup \{x^i\} ) \sthat  
  \exists \trace, \trace' \in \tdom, \qval, \qval'\sthat  
 \env(\trace) z = \env(\trace') z \land 
 ...
 then  $x \in VAR(\qexpr) \land i = \llabel(\trace) x$.
	...
	\[
		...
		\\ \quad
		\forall z^j \in \lvar \setminus \diff, \trace, \trace' \in \tdom, \qval, \qval' \sthat  
		\\ \quad
		\env(\trace) z = \env(\trace') z \land 
		\config{\trace, \qexpr} \qarrow \qval \land \config{\trace', \qexpr} \qarrow \qval' \land \qval =_q \qval'
		\\ \quad
...
Taking an arbitrary set of labelled variables 
	...
	$,
	\\
	Let $\trace, \trace' \in \tdom, v, v'$ be the two traces and values satisfies hypothesis $(2)$.
	\\
...
\\
$\forall z^j \in \lvar \setminus \{x^i\}, \trace, \trace' \in \tdom, v, v' \sthat  
\env(\trace) z = \env(\trace') z \land 
\config{\trace, \aexpr} \aarrow v \land \config{\trace', \aexpr} \aarrow v' \land v = v'$.
\\
Then with the hypothesis $(1)$, we know:
\\
$\forall z^j \in \lvar \setminus (\diff \cup \{x^i\} ), \trace, \trace' \in \tdom, v, v'\sthat  
\env(\trace) z = \env(\trace') z \land 
\config{\trace, \aexpr} \aarrow v \land \config{\trace', \aexpr} \aarrow v' \land v = v'$
\\
...
\label{lem:inv_event}
For all $c\in \cdom, \trace_0 \in \tdom, \event \in \eventset$such that 
...
\begin{itemize}
	...
	\begin{itemize}
	 \item there exists $\trace_1' \in \tdom, c' \in \cdom, \expr$ such that
\[
\begin{array}{l}
...
\end{array}
\]
\item or there exists $\trace_1' \in \tdom, c' \in \cdom, \qexpr$ such that 
\[
\begin{array}{l}
...
\begin{itemize}
\item there exists $\trace_1' \in \tdom, c', c_t, c_f, c'' \in \cdom, \bexpr$ such that
\[
\begin{array}{l}
	% \big( 
	% 	\exists \trace_1' \in \tdom, c', c_t, c_f, c'' \in \cdom, \bexpr \sthat 
		\config{c, \trace_0} \rightarrow^* \config{\eif ([b]^l, c_t, c_f);c', \trace_0 \tracecat \trace_1'} \rightarrow^{if-b}
		\config{c'', \trace_0 \tracecat \trace_1'\tracecat [\event] } \rightarrow^{*}
		\config{\eskip, \trace_0 \tracecat \trace_1} 
\end{array}
\]
\item or there exists $ \trace_1' \in \tdom, c', c_w, c'' \in \cdom, \bexpr$ such that 
\[
...
\[
	\begin{array}{l}
	\forall c \in \cdom , \trace_0 \in \ftdom_0(c), \trace \in \tdom,x^l \in \lvar(c) \sthat 
	\config{c, \trace_0} \xrightarrow{}^* \config{c', \trace}
	\land
