...
\begin{proof}
    Let $\event = (x, l, v, \qval) \in \eventset$ be arbitrary event that  is generated from a query request for some variable $x$, label $l$ and a query result $v$.
    \\
    Let $ \trace \in \ftdom$ be the trace that contains $\event$ and $\trace = \trace_1 \tracecat [\event] \tracecat \trace_2 = $ for some finite traces
    $\trace_1 \tracecat [\event] \tracecat \trace_2 \in \ftdom$.
    \\
