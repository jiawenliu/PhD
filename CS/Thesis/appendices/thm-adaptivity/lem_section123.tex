  ...
  Both of the two cases will generate event of the evaluation result on the guard expression
  $\clabel{b}^l$.
  \\
  By rule \rname{if-t} and \rname{{if-f}},
  the event $\event $ is either $(b, l_w, \etrue, \bullet)$ or $(b, l, \efalse, \bullet)$ respectively.
  \\
  Again by rule \rname{if-t} and \rname{{if-f}}, $\event $ is appended to the initial trace.
  \\
  In this sense, let $\trace' =  [(b, l_w, \efalse, \bullet)]$
  or $\trace' =  [(b, l_w, \efalse, \bullet)]$ respectively,
  we have 
  $\trace_0 \tracecat \trace' = \trace$ in both cases and this case is proved.
  % \\
