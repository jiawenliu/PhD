...
\[
  \begin{array}{l}
    \forall c \in \cdom, \vtrace_0 \in \ftdom_0(c), \trace \in \ftdom ,  l, l' \in \absV(c), \event = (\_, l, \_) \in \eventset \sthat 
...
\begin{proof}
  Taking arbitrary $\trace_0 \in \ftdom$, and an arbitrary event $\event = (\_, l, \_) \in \eventset$, it is sufficient to show:
  \[
  \begin{array}{l}
    \forall \trace \in \ftdom \sthat 
\config{{c}, \trace_0} \to^{*} \config{\clabel{\eskip}^{l'}, \trace_0 \tracecat \vtrace} 
\land \event \in \trace 
\\
\qquad \implies 
...
\]
  ...
  $.
  \\
  Let $\trace_w \in \ftdom$ satisfying following execution:
  \\
  $
  \config{{
  c_w,
...
$ for some $\trace_1 \in \ftdom$ and $\trace = [(b, l_w, \etrue)] \tracecat \trace_w \tracecat \trace_1$.
\\
...
(3) $\event \in \trace_1$.
  \\
In case of (1). $\event \eventeq (b, l_w, \etrue)$, since $\absflow(c) = \absflow'(c;\clabel{\eskip}^{l_e}) = \{(l, \top, \init(c_w))\} \cup \ldots $, we have $\absevent = (l, \top, \init(c_w))$ and this case is proved.
\\
...
$c_w$ with the execution 
  ...
  \subcaseL{(2.1)}
  Since $\absflow(c) = \absflow'(c_w) \cup \{(l', dc, l_w)| (l', dc) \in \absfinal(c_w) \} \cup \ldots $, 
  ...
  \\
  %
In case of (3). $\event \in \trace_1$, we know either $\event = (b, l_w, \_)$, or $\event \in \trace_w'$ where $\trace_w' \in \ftdom$ is the trace of executing $c_w$ in an iteration.
\\
Then this case is proved by repeating the proof in case (1) and case (2).
  ...
  \\
  By the abstract execution trace computation, $\absflow(c) = \{(l, \top, \init(c_w))\} \cup \ldots $, 
...
\subsection{Proof of Theorem~\ref{thm:transition_bound_sound}}
  ...
  % \forall \vtrace, \trace' \in \ftdom \sthat  
