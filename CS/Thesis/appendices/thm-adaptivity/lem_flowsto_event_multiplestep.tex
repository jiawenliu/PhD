\begin{lem}[Multi-step Soundness through Assignment Event]
	...
	% 	\item
		...
		\begin{array}{l}
			...
			\\ \quad 
			\implies 
			\flowsto(\pi_1(\event_1)^{\pi_2(\event_1)}, \pi_1(\event_2)^{\pi_2(\event_2)}, c) \lor
			\\ \quad 
			\Big( 
...
%
\end{proof}
\begin{lem}[Multi-step Soundness through Testing Event]
...
\begin{lem}[Equation~\ref{eq:eventdep}(c) Inversion]
	...
	satisfy the Equation~\ref{eq:eventdep}(c) in Definition~\ref{def:event_dep} and
	...
	$\event_b \in \eventset^{\test}$ is the testing event witness,
...
%  \rightarrow^{*} 
  ...
  \config{(c_t;c_3' / c_f;c_3') , 
  \trace_1 \tracecat [\event_1] \tracecat \trace \tracecat [\event_b]} 
  \\
  \qquad 
  /  \rightarrow^{\rname{while-b}} 
  \config{(c_w; \ewhile [b]^{l_b} \edo (c_w);c_3'/\clabel{\eskip}^{l'}; c_3'), 
  \trace_1 \tracecat [\event_1] \tracecat \trace \tracecat [\event_b]} 
  \\
  \qquad  \rightarrow^{*} 
...
In this execution, the conditional command $\eif ([b]^{l_b}, c_t, c_f)$ or $\ewhile [b]^{l_b} \edo (c_w)$ 
% is the conditional command of the assignment commands associated to the 
is the inversion result from
$\event_b$ according to Inversion Lemma~\ref{lem:inv_event} of this testing event.
\\
% \[\begin{array}{l}   
% 	  \config{\eif ([b]^{l_b}, c_t, c_f) / \ewhile [b]^{l_b} \edo (c_w);{c}_3', 
% 	  \vtrace_1 \tracecat [\event_1] \tracecat \trace} 
...
$$
%
The same for case of $\eif ([b]^{l_b}, c_t, c_f)$ 
