\begin{example}[twoRounds]
    We illustrate the key technical components of our framework again through the same simple adaptive data analysis with two rounds of adaptivity as Figure~\ref{fig:overview-example}(a).
    Again, in this analysis, an analyst asks $k+1$ queries to a mechanism in two phases, $k$ queries in the first round and a new query constructed based on the results of the previous $k$ queries in the second round.
...
%    \end{centering}
%    \end{subfigure}
\caption{(a) The same example program $\kw{towRounds(k)}$ as Figure~\ref{fig:overview-example}(a), an example 
with two rounds of adaptivity (b) The corresponding semantics-based dependency graph.
}
