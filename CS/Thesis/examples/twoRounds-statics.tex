\begin{example}[twoRounds]    
    ...
    The estimated dependency graph that our static analysis algorithm returns for the program $\kw{twoRounds(k)}$ is in Figure~\ref{fig:twoRounds}(a).
    ...
    For the $\kw{twoRounds(k)}$ example, every edge in Figure~\ref{fig:twoRounds-adaptfun}(b) is precisely inferred by our combined analysis, this is why Figure~\ref{fig:twoRounds}(c) contains exactly the same edges.
    ...
    For example, consider the vertex $x^{3}$, its weight is $k$ and this provides an upper bound on the values returned by the weight function $\lambda \trace. \rho(\trace)k$ associated with vertex $x^{3}$ in Figure~\ref{fig:twoRounds}(b) for any initial state. 
