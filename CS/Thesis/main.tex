...
\reader{First}{Marco Gaboardi, Ph.D.}{Associate Professor of Computer Science}
\reader{Second}{Alley Stoughton, Ph.D.}{Research Professor of Computer Science}
\reader{Third}{Assaf J. Kfoury, Ph.D.}{Professor of Computer Science}
\reader{Fourth}{Hongwei Xi, Ph.D.}{Associate Professor of Computer Science}
...
\input{chapters/introduction/methodology}
...
\clearpage
\chapter{The Language Model -- {\tt QWhile}}
\label{sec:adapt-language}
\input{chapters/adapt/language}

\clearpage
\chapter{The Adaptivity Formalization}
\label{sec:adapt-exe}
\input{chapters/adapt/formalization-preface}

\section{May-dependency between Variables}
\label{sec:dynamic-datadep}
\input{chapters/adapt/var-dep}

\section{Semantics-based Dependency Graph}
\label{sec:dynamic-graph}
\input{chapters/adapt/semantic-graph}

\section{The Adaptivity}
\label{sec:dynamic-adapt}
\input{chapters/adapt/adaptivity-definition}

\section{Examples}
\label{sec:dynamic-examples}
\input{chapters/adapt/formalization-examples}%
\clearpage

\chapter{The Program Analysis for Adaptivity}
\label{sec:adapt-static}
\input{chapters/adapt/adaptfun-preface}

\section{Algorithm Guideline}
\label{sec:static-overview}
\input{chapters/adapt/adaptfun-guide}

\section{Graph Vertex and Annotation Estimation}
\label{sec:alg_vertexgen}
\input{chapters/adapt/adaptfun-vertexgen}


\section{Graph Edge and Weight Estimation}
\label{sec:alg_weightedgegen}
\input{chapters/adapt/adaptfun-weightedgegen}

\section{Dependency Graph Estimation}
\label{sec:alg_graphgen}
\input{chapters/adapt/adaptfun-graphgen}

\section{Adaptivity Upper Bound Computation}
\label{sec:alg_adaptcompute}
\input{chapters/adapt/adaptfun-adaptcompute}

\section{Examples}
\label{sec:static-examples}
\input{chapters/adapt/adaptfun-examples}


\clearpage
\chapter{Examples and Evaluation Results}
...
\chapter*{\redd{PART \romannum{2} \quad  REACHABILITY BOUND ANALYSIS}}
\input{chapters/reachability/preface}
\chapter{Introduction}
\label{sec:reachability-intro}

\section{Background}
\label{sec:reachability-background}
\input{chapters/reachability/intro-background}

\section{Motivation}
\label{sec:reachability-motivation}
\input{chapters/reachability/intro-motivation}

\section{Technique Overview}
\label{sec:reachability-overview}
\input{chapters/reachability/intro-overview}

\section{Outline and Contributions}
\label{sec:reachability-outline}
\input{chapters/reachability/intro-outline}

% \chapter{Path-Sensitive Reachability-bound Analysis}
% \label{sec:reachability-analysis}

\chapter{{The Language Model}}
\label{sec:psRB-language}
\input{chapters/reachability/language}
% %
\chapter{{The Reachability-bound}}
\label{sec:psRB-definition}
\input{chapters/reachability/rb_definition}


\chapter{The Path-sensitive Reachability-bound Analysis}
\label{sec:psRB-analysis-alg}
% \input{chapters/reachability/alg-preface}

\section{Main Steps}
\label{sec:psRB-preface}
\input{chapters/reachability/alg-mainstep}

\section{Abstraction Transition Graph}
\label{sec:psRB-progabs}
\input{chapters/reachability/alg-progabs}

\section{Loop Refinement}
\label{sec:psRB-refine}
\input{chapters/reachability/alg-refine}

\section{Ranking Function Computation and Estimation}
\label{sec:psRB-rank}
\input{chapters/reachability/alg-rank}

\section{Path-sensitive Reachability-bound Algorithm Overview}
\label{sec:psRB-psrb}
\input{chapters/reachability/alg-psrb}

\section{Loop Bound and Local Path Reachability-bound Computation}
\label{sec:psRB-pathlocalrb}
\input{chapters/reachability/alg-pathlocalrb}

\section{Loop Reachability-bound Computation}
\label{sec:psRB-looprb}
\input{chapters/reachability/alg-looprb}

\section{Path Reachability-bound Computation}
\label{sec:psRB-pathrb}
\input{chapters/reachability/alg-pathrb}

\section{Reachability-bound Computation}
\label{sec:psRB-rb}
\input{chapters/reachability/alg-rb}

\chapter{Example And Evaluation}
\label{sec:psRB-eval}
\section{Examples}
\label{sec:psRB-example}
\input{chapters/reachability/examples}

\section{Empirical Evaluation}
\label{sec:psRB-implementation}
\input{chapters/reachability/eval}

\chapter{Related Works}
\label{sec:psRB-relatedwork}
\input{chapters/reachability/relatedwork}
...
\input{chapters/conclusion/preface}

\section{Program Adaptivity Analysis}
\label{sec:conclusionfuture-adapt}

\subsection{Conclusion}
\label{sec:conclusion-adapt}
\input{chapters/conclusion/conclusion-adapt}

\subsection{Future Works}
\label{sec:future-adapt}
\input{chapters/conclusion/future-adapt}

\section{Path-sensitive Reachability-bound Analysis}
\label{sec:conclusionfuture-psRB}
\subsection{Conclusion}
\label{sec:conclusion-psRB}
\input{chapters/conclusion/conclusion-psRB}
\subsection{Future Works}
\label{sec:future-psRB}
\input{chapters/conclusion/future-psRB}

% \section{Towards Non-Monotonic Resource Cost Program Analysis}
% \label{sec:future-cost}
% \input{chapters/conclusion/future-nonmonotonic_cost}
...
\begin{appendices}
\chapter{Supplementary Materials in {PART \romannum{1}}}
\label{apdx:adapt}
\input{chapters/adapt/appendix}

\chapter{Supplementary Materials in {PART \romannum{2}}}
\label{apdx:reachability}
\input{chapters/reachability/appendix}
...
% keep this on, or you will get warnings about undefined citations
...
Boston, MA 02115  \\ 
(716) 930 6471 \\
\hyperlink{mailto:jiawenliu18@gmail.com}{personal email}\\
 CDS 1002, \\ Boston University \\ Boston, MA 02118 \\
\hyperlink{mailto:jiawenl@bu.edu}{working email}\\
}
\end{center}}
\end{multicols}
...
05/2023\small(expected) &  Ph.D. Boston University, Boston, MA; Computer Science\\
05/2023\small(expected) &  M.S. Boston University, Boston, MA; Computer Science\\
...
\textbf{Title}: & Program Analysis for Quantitative-reachability Properties \\
\textbf{Thesis advisor}: & Marco Gaboardi, PhD\\
\textbf{Defense date}: & March 30, 2023 \\
\textbf{Summary}: & 
Reasoning about program's properties to control program's execution behavior
has great potential in 
program optimization and program security. 
This dissertation exploits the program analysis to reason about two \emph{quantitative-reachability} properties with application in different research areas.
\end{tabular}
